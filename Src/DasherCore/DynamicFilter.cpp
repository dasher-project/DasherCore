...
bool CDynamicFilter::OneStepTowards(CDasherModel *pModel, myint X, myint Y, unsigned long iTime, double dSpeedMul) {
  ...
  pModel->ScheduleOneStep(Y-X, Y+X, iSteps, GetLongParameter(Parameters::LP_X_LIMIT_SPEED), GetBoolParameter(Parameters::BP_EXACT_DYNAMICS));
  return true;
}
...
double CDynamicFilter::FrameSpeedMul(CDasherModel *pModel, unsigned long iTime) {
  ...
  if(GetBoolParameter(Parameters::BP_SLOW_START)) {
    if ((iTime - m_iStartTime) < GetLongParameter(Parameters::LP_SLOW_START_TIME))
      d *= 0.1 * (1 + 9 * ((iTime - m_iStartTime) / static_cast<double>(GetLongParameter(Parameters::LP_SLOW_START_TIME))));
  }
