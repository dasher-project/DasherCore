...
bool CTwoBoxStartHandler::DecorateView(CDasherView *pView) {
  ...
  int iMousePosDist = GetLongParameter(Parameters::LP_MOUSEPOSDIST);
...
void CTwoBoxStartHandler::Timer(unsigned long iTime, dasherint iDasherX, dasherint iDasherY, CDasherView *pView) { 
  ...
  int iBoxMin, iBoxMax;
  if(m_bFirstBox) {
    iBoxMax = pView->Screen()->GetHeight() / 2 - (int)GetLongParameter(Parameters::LP_MOUSEPOSDIST) + 50;
    iBoxMin = iBoxMax - 100;
  }
  else {
    iBoxMin = pView->Screen()->GetHeight() / 2 + (int)GetLongParameter(Parameters::LP_MOUSEPOSDIST) - 50;
    iBoxMax = iBoxMin + 100;
  }
