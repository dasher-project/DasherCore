...
CDasherViewSquare::CDasherViewSquare(CSettingsUser* pCreateFrom, CDasherScreen* DasherScreen, Opts::ScreenOrientations orient)
	: CDasherView(DasherScreen, orient), CSettingsUserObserver(pCreateFrom), m_Y1(4), m_Y2(static_cast<const myint>(0.95 * CDasherModel::MAX_Y)), m_Y3(static_cast<const myint>(0.05 * CDasherModel::MAX_Y)), m_bVisibleRegionValid(false)
{
                                       ...
                                       CExpansionPolicy& policy)
{
	...
	for (auto it = m_DelayedTexts.begin(), E = m_DelayedTexts.end(); it != E; ++it)
		DoDelayedText(*it);
...
void CDasherViewSquare::Circle(myint Range, myint y1, myint y2, int fCol, int oCol, int lWidth)
{
	...
	if (y1 < iDasherMinY)
	{
		...
		x1 = std::min(iDasherMaxX, static_cast<myint>(sqrt(double(r * r - sq(cy-iDasherMinY)))));
		y1 = iDasherMinY;
	}
	else
	{
	...
	{
		//intersect...
		x2 = std::min(iDasherMaxX, static_cast<myint>(sqrt(double(r * r - sq(iDasherMaxY-cy)))));
...
#define NUM_STEPS 40
	...
	VisibleRegion(minX, minY, maxX, maxY);
	{
		...
		for (int i = 0; i <= NUM_STEPS; i++)
		{
			...
			Dasher2Screen(std::min(maxX, static_cast<myint>(of * of * x1 + 2.0 * of * f * x2 + f * f * x3)), std::max(minY, std::min(maxY, static_cast<myint>(of * of * y1 + 2.0 * of * f * y2 + f * f * y3))), p_array[i].x, p_array[i].y);
		}
	}
	{
		...
		{
			double f = i / static_cast<double>(NUM_STEPS), of = 1.0 - f;
			Dasher2Screen(std::min(maxX, static_cast<myint>(of * of * x1 + 2.0 * of * f * x2 + f * f * x3)), std::max(minY, std::min(maxY, static_cast<myint>(of * of * y1 + 2.0 * of * f * y2 + f * f * y3))), p_array[i + NUM_STEPS + 1].x, p_array[i + NUM_STEPS + 1].y);
		}
	}
                                       ...
                                       CDasherNode*& pOutput)
{
	...
	else
	{
		...
		if (!pRender->onlyChildRendered)
		{
			...
			if (lasty < std::min(y2, iDasherMaxY))
			{
...
beginning:
	...
	if (CDasherNode* pChild = pRender->onlyChildRendered)
	{
		...
		myint newy2 = y1 + (Range * pChild->Hbnd()) / CDasherModel::NORMALIZATION;
		if (
			(newy1 < iDasherMinY && newy2 > iDasherMaxY))
		{
	...
	while (I != E)
	{
		...
		if (newy1 <= iDasherMaxY && newy2 >= iDasherMinY)
		{
			//onscreen
...
void CDasherViewSquare::Screen2Dasher(screenint iInputX, screenint iInputY, myint& iDasherX, myint& iDasherY)
{
	...
	case Opts::LeftToRight:
		...
		break;
	case Opts::RightToLeft:
		iDasherX = static_cast<myint>((iInputX) * SCALE_FACTOR / iScaleFactorX);
		iDasherY = static_cast<myint>(CDasherModel::MAX_Y / 2 + (iInputY - iScreenHeight / 2) * SCALE_FACTOR / iScaleFactorY);
		break;
	case Opts::TopToBottom:
		iDasherX = static_cast<myint>((iScreenHeight - iInputY) * SCALE_FACTOR / iScaleFactorX);
		iDasherY = static_cast<myint>(CDasherModel::MAX_Y / 2 + (iInputX - iScreenWidth / 2) * SCALE_FACTOR / iScaleFactorY);
		break;
	case Opts::BottomToTop:
		iDasherX = static_cast<myint>((iInputY) * SCALE_FACTOR / iScaleFactorX);
		iDasherY = static_cast<myint>(CDasherModel::MAX_Y / 2 + (iInputX - iScreenWidth / 2) * SCALE_FACTOR / iScaleFactorY);
		break;
	default:
		break;
...
void CDasherViewSquare::SetScaleFactor(void)
{
	...
	const bool bHoriz(GetOrientation() == Opts::LeftToRight || GetOrientation() == Opts::RightToLeft);
...
void CDasherViewSquare::Dasher2Screen(myint iDasherX, myint iDasherY, screenint& iScreenX, screenint& iScreenY)
{
	...
	{
	case Opts::LeftToRight:
		iScreenX = static_cast<screenint>(iScreenWidth -
	...
	case Opts::TopToBottom:
		iScreenX = static_cast<screenint>(iScreenWidth / 2 +
			CustomIDivScaleFactor((iDasherY - CDasherModel::MAX_Y / 2) * iScaleFactorY));
		iScreenY = static_cast<screenint>(iScreenHeight -
			CustomIDivScaleFactor(iDasherX * iScaleFactorX));
		break;
	case Opts::BottomToTop:
		iScreenX = static_cast<screenint>(iScreenWidth / 2 +
	...
	if (x1 != x2 && y1 != y2)
	{
		...
		if (GetLongParameter(LP_NONLINEAR_X) && (x1 > m_iXlogThres || x2 > m_iXlogThres))
		{
			...
			for (;;)
			{
				...
				if (GetOrientation() == Opts::LeftToRight || GetOrientation() == Opts::RightToLeft)
				{
