...
bool Less(std::pair<double, CDasherNode*> x, std::pair<double, CDasherNode*> y) {
    return x.first < y.first;
}
bool More(std::pair<double, CDasherNode*> x, std::pair<double, CDasherNode*> y) {
    return x.first > y.first;
}

BudgettingPolicy::BudgettingPolicy(CDasherModel* pModel, unsigned int iNodeBudget)
    : CExpansionPolicy(pModel), m_iNodeBudget(iNodeBudget) {}
...
bool BudgettingPolicy::apply() {
    ...
    while (!sCollapse.empty() && static_cast<unsigned int>(currentNumNodeObjects()) > m_iNodeBudget) {
    ...
    while (!sExpand.empty() && sExpand.back().first > collapseCost) {
        if (static_cast<unsigned int>(currentNumNodeObjects() + sExpand.back().second->ExpectedNumChildren()) <
            m_iNodeBudget) {
            ExpandNode(sExpand.back().second);
            sExpand.pop_back();
            bReturnValue = true;
            //...and loop.
        } else if (!sCollapse.empty() && sCollapse.back().first < sExpand.back().first) {
            ...
            break; // not enough room, nothing to collapse.
    }
    sExpand.clear();
    sCollapse.clear();
    return bReturnValue;
...
AmortizedPolicy::AmortizedPolicy(CDasherModel* pModel, unsigned int iNodeBudget)
    : BudgettingPolicy(pModel, iNodeBudget), m_iMaxExpands(std::max(1u, (500 + iNodeBudget) / 1000)) {}

AmortizedPolicy::AmortizedPolicy(CDasherModel* pModel, unsigned int iNodeBudget, unsigned int iMaxExpands)
    : BudgettingPolicy(pModel, iNodeBudget), m_iMaxExpands(iMaxExpands) {}
...
void AmortizedPolicy::trim() {
    ...
    for (vector<pair<double, CDasherNode*>>::iterator it1 = sExpand.begin(), it2 = backup.begin(); it1 != sExpand.end();
         it1++, it2++)
        if (*it1 == *it2)
            continue;
        else if (it1->first == dFirstCost && it2->first == dFirstCost)
            continue;
        else
            cout << "trim not equal!\n";
#endif
