...
Dasher::CSocketInputBase::CSocketInputBase(CSettingsUser* pCreator, CMessageDisplay* pMsgs)
    ...
    SetRawRange(0, ((double)GetLongParameter(LP_SOCKET_INPUT_X_MIN)) / 1000.0,
                ((double)GetLongParameter(LP_SOCKET_INPUT_X_MAX)) / 1000.0);
    SetRawRange(1, ((double)GetLongParameter(LP_SOCKET_INPUT_Y_MIN)) / 1000.0,
                ((double)GetLongParameter(LP_SOCKET_INPUT_Y_MAX)) / 1000.0);
...
Dasher::CSocketInputBase::~CSocketInputBase() {
    //  Would like to call StopListening(); Can't do this here because by the time this (base class) destructor is
    //  called,
            ...
            if (strncmp(coordinateNames[i], message, len) == 0) {
                ...
                if (sscanf(message + len, "%lf", &rawdouble) == 1) {
                    ...
                    if (rawdouble < actualMin) {
                        ...
                        cerr << "Socket input: clipped " << coordinateNames[i] << " value of " << rawdouble
                             << "to configured minimum of " << actualMin << endl;
                        rawdouble = actualMin;
                    }
                    if (rawdouble > actualMax) {
                        // TODO: Should these be converted to calls to Message() ? On first occurrence only???
                        cerr << "Socket input: clipped " << message << " value of " << rawdouble
                             << "to configured maximum of " << actualMax << endl;
                        rawdouble = actualMax;
                    }
                    ...
                    if (do_lowpass) {
                        ...
                        double newcoord = ((rawdouble - rawMinValues[i]) / (rawMaxValues[i] - rawMinValues[i]) *
                                           dasherMaxCoordinateValues[i]);
                        dasherCoordinates[i] =
                            (myint)((1 - 1 / timeconst) * (double)dasherCoordinates[i] + (1 / timeconst) * newcoord);
                    } else {
                        // straightforward linear mapping to dasher coordinates:
                        // Treat X coordinate specially: reverse sense so it has the more intuitive left-to-right
                        // direction
                        ...
                        if (max != min) { // prevent nasty explosion
                            dasherCoordinates[i] =
                                (myint)((rawdouble - min) / (max - min) * (double)dasherMaxCoordinateValues[i]);
                        }
                    }

                    SocketDebugMsg("Socket input: new value for coordinate %d rescales to %u in Dasher's internal "
                                   "coordinates (range 0-%d).",
                                   i, (unsigned int)dasherCoordinates[i], (int)dasherMaxCoordinateValues[i]);
