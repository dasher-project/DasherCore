...
CRoutingPPMLanguageModel::CRoutingPPMLanguageModel(CSettingsStore* pSettingsStore, const std::vector<symbol>* pBaseSyms,
                                                   const std::vector<std::set<symbol>>* pRoutes,
                                                   bool bRoutesContextSensitive)
    : CAbstractPPM(pSettingsStore, static_cast<int>(pRoutes->size()) - 1, new CRoutingPPMnode(-1),
                   pSettingsStore->GetLongParameter(LP_LM_MAX_ORDER)),
      NodesAllocated(0), m_NodeAlloc(8192), m_pBaseSyms(pBaseSyms), m_pRoutes(pRoutes),
      m_bRoutesContextSensitive(bRoutesContextSensitive) {
...
void CRoutingPPMLanguageModel::GetProbs(Context context, std::vector<unsigned int>& probs, int norm,
    ...
    for (CPPMnode* pTemp = ppmcontext->head; pTemp; pTemp = pTemp->vine) {
        ...
        if (iTotal) {
            ...
            for (ChildIterator it = pTemp->children(); it != pTemp->end(); it++) {
                unsigned int p =
                    static_cast<myint>(size_of_slice) * (100 * (*it)->count - beta) / (100 * iTotal + alpha);
...
symbol CRoutingPPMLanguageModel::GetBestRoute(Context ctx) {
    ...
    for (CPPMnode* pTemp = context->head; pTemp != m_pRoot; pTemp = pTemp->vine) {
        ...
        for (std::map<symbol, unsigned short int>::const_iterator it = node->m_routes.begin();
             it != node->m_routes.end(); it++) {
            unsigned int p = static_cast<unsigned int>(static_cast<unsigned long>(size_of_slice) *
                                                       (100 * it->second - beta) / (100 * iTotal + alpha));
            iToSpend -= p;
            probs[it->first] += p;
        }
    }
