...
class CCTWLanguageModel : public CLanguageModel {
  public:
    ...
    int MaxCount;   // The maximum number of a and b, the counts of zeros and ones in each CCTWnode, before they are
                    // halved.
    ...
    class CCTWNode {
      public:
        ...
        unsigned char NrTries; // needed to check if the correct node is found by hashing. MaxTries is bounded, 4-5 bits
                               // would suffice
    ...
    using CLanguageModel::ReadFromFile;
    using CLanguageModel::WriteToFile;
