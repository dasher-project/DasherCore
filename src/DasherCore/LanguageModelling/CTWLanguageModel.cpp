...
CCTWLanguageModel::CCTWLanguageModel(int iNumSyms) : CLanguageModel(iNumSyms) {
    ...
    MaxNrNodes =
        4194304;    // Max number of CCTWNodes in the table, trade-off between compression and memory usage. 2^22 = 4M
    ...
    // Fill RootIndex table with indices of the RootNodes <- now I round up to next power of 2, should only create for
    // possible symbols Does that make a noticable difference in memory usage? Rootnodes with no symbols associated will
    // accumulate no counts, so they only cost 1 node each (6 bytes). Does it waste codespace? Do rootnodes with no
    // symbols associated with them still get assigned a positive probability? Set NrTries to max+1, to identify a
    // RootNode
    for (int i = 0; i < (1 << NrPhases); i++) {
        RootIndex[i] = HashTable.GetHashOffSet(i) &
                       (MaxNrNodes -
                        1); // MaxNrNodes is a power of 2, & results in a mod operation, walk 'round' through the array
        Tree[RootIndex[i]].NrTries =
            MaxTries + 1; // in rootnodes the character value doesn't matter, as long as Tries = unique
        TotalNodes++;
    }
}
...
inline void CCTWLanguageModel::Scale(uint64_t& a, uint64_t& b) {
    ...
    while ((a > MaxValue) | (b > MaxValue)) { // scale to right
        a = a >> 1;
        b = b >> 1;
    }
    while ((a < (MaxValue >> 1)) & (b < (MaxValue >> 1))) { // scale to left
        a = a << 1;
        b = b << 1;
    }
    // prevent 0
...
void CCTWLanguageModel::UpdatePath(
    int bit, int Update, int ValidDepth, int*& index, unsigned short int& P0,
    unsigned short int& P1) { // updates the CTW data of the nodes in 'index' with value of 'bit'.
                              // Update specifies yes (1) or no (0) (GetProbs). In the case 'no', the new Pws are
                              // calculated but the tree is not altered in any way
    ...
    {                                          // could do more fancy things here
        ...
        {                // first update counts
            if (bit) {
                if (CountOne == MaxCount) { // half counts
            ...
            {
                if (CountZero == MaxCount) { // half counts
                    CountZero = (CountZero + 1) / 2;
                    CountOne = (CountOne + 1) / 2;
                } else
                    CountZero++;
...
int CCTWLanguageModel::FindPath(
    CCTWContext& context, char NewChar, int phase, int create,
    int*& index) { // Puts the Tree-array indices of the CCTWNodes on the path of Context in index.
    ...
    for (unsigned int i = 0; i < context.Context.size(); i++) {
        ...
        Stepsize = (CHashTable::GetHashOffSet(CurChar) << 1) +
                   1; // get stepsize. Shift+1 to keep result odd, to prevent cycles
        bool found = false;
        for (int Tries = 1; Tries < MaxTries; Tries++) {
            ...
            { // see if this is the correct node: compare tries and last (current) character
                if (Tree[curindex].Symbol == CurChar) // node found
                {
                    ...
                    index[i + 1] =
                        curindex; // tell calling function where to find the node, i+1 because index[0] = rootnode
            ...
            if (Tree[curindex].NrTries == 0) // empty node found, create new node
            {
                ...
                if (!Frozen) // Still space in the tree
                {
                    ...
                    if ((float)(TotalNodes) / (float)(MaxNrNodes) >
                        MaxFill) // Max fillratio of tree reached, freeze tree
                        Frozen = true;
                    found = true; // to avoid 'failed'
                    index[i + 1] =
                        curindex; // tell calling function where to find the node, i+1 because index[0] = rootnode
...
void CCTWLanguageModel::EnterSymbol(
    ...
    if (Context.Context.size() == MaxDepth) Context.Full = true;
}
...
void CCTWLanguageModel::LearnSymbol(Context CurContext, int Symbol) {
    ...
    {                         // find indices of the tree nodes corresponding to the context
        ...
        for (int phase = 0; phase < NrPhases; phase++) {
            ValidDepth =
                FindPath(Context, Symbol, phase, 1, Index); // Find indices of the nodes for this phase and context
...
void CCTWLanguageModel::GetProbs(Context context, std::vector<unsigned int>& Probs, int Norm, int iUniform)
    const { // because we reuse findpath and updatepath function, we need to de-const the object :(
    ...
    vector<unsigned short int> Interval((1 << (NrPhases + 1)) -
                                        1); // number of rootnodes*2 (1 prob for bit 0 and 1 each)
    if (Norm > 65535) {
        Interval[0] = 65535;  // to prevent overflow
        pLeft = Norm - 65535; // if Norm is way bigger than 2^16 - 1, uniformly distributing the 'leftover' could still
                              // cause overflow
    } else
        Interval[0] = Norm;
    ...
    for (int phase = 0; phase < NrPhases; phase++) {
        ...
        for (int steps = 0; steps < 1 << phase; steps++) { // find the path for all needed symbols
            // FIXME now I round up to next power of 2
            ValidDepth = self->FindPath(LocalContext, steps * stepsize, phase, 0,
                                        Index); // Find indices of the nodes for this phase and context

            IntervalB = Interval[(1 << phase) + steps - 1];
            self->UpdatePath(0, 0, ValidDepth, Index, Pw0, Pw1);

            IntervalZ = (IntervalB * Pw0) /
                        (uint64_t)(Pw0 + Pw1); // flooring, influence of flooring P0 instead of P1 is negligible
            IntervalO = IntervalB - IntervalZ;

            MinInterval =
                MinProb * 1 << (NrPhases - 1 -
                                phase); // leafs for each rootnode at the current phase, assuming a full alphabet!!
...
bool CCTWLanguageModel::WriteToFile(std::string strFilename, std::string AlphabetName) {
    ...
    GenericHeader.iHeaderSize =
        sizeof(SLMFileHeader) +
        static_cast<unsigned short>(
            AlphabetName.length()); // Total size of header (including variable length alphabet name)

    FILE* OutputFile;
    OutputFile = fopen(strFilename.c_str(), "wb");
    if (OutputFile) {
        // write header
        fwrite(GenericHeader.szMagic, sizeof(GenericHeader.szMagic[0]), sizeof(GenericHeader.szMagic) - 1,
               OutputFile); // Do not print Null-Char
        ...
        fwrite(AlphabetName.c_str(), sizeof(AlphabetName[0]), AlphabetName.length(),
               OutputFile); // UTF-8 encoded alphabet name (variable length struct)
...
bool CCTWLanguageModel::ReadFromFile(std::string strFilename, std::string AlphabetName) {
    ...
    if (InputFile) {
        ...
        TODO: Checking of the SLMFileHeader, which is not specific to the CTW languagemodel should be done in
        DasherModel, only CTW specific information (MaxNrNodes) should be checked here. The values to compare with
        should be parameters and not hardcoded. */
        ...
        if (bytesRead < sizeof(GenericHeader.iLMID) ||
            GenericHeader.iLMID != 5) { // header indicates this is not a CTW model
            return false;
        }
        ...
        if (bytesRead < sizeof(GenericHeader.iLMMinVersion) ||
            GenericHeader.iLMMinVersion > 1) { // header indicates stored model newer than we can handle
            return false;
        }

        bytesRead = fread(&GenericHeader.iAlphabetSize, sizeof(GenericHeader.iAlphabetSize), 1, InputFile);
        if (bytesRead < sizeof(GenericHeader.iAlphabetSize) ||
            GenericHeader.iAlphabetSize !=
                GetSize()) { // header indicates stored model uses an alphabet of different size
            return false;
        }

        ReadAlphabetName = new char[GenericHeader.iHeaderSize - sizeof(SLMFileHeader) + 1];
        bytesRead = fread(ReadAlphabetName, sizeof(ReadAlphabetName[0]),
                          GenericHeader.iHeaderSize - sizeof(SLMFileHeader), InputFile);
        ...
        if (bytesRead < sizeof(ReadNrNodes) ||
            ReadNrNodes != MaxNrNodes) { // header indicates different number of nodes in the hashtable
            return false;
        }
        ...
        for (int i = 0; i < MaxNrNodes; i++) {
            ...
            if (fread(&Tree[i].NrTries, sizeof(CCTWNode::NrTries), 1, InputFile) < sizeof(CCTWNode::NrTries))
                return false;
            if (fread(&Tree[i].Pe, sizeof(CCTWNode::Pe), 1, InputFile) < sizeof(CCTWNode::Pe)) return false;
            if (fread(&Tree[i].PwChild, sizeof(CCTWNode::PwChild), 1, InputFile) < sizeof(CCTWNode::PwChild))
                return false;
        }
        fclose(InputFile);
        return true;
