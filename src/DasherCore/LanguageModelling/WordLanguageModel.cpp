...
CWordLanguageModel::CWordLanguageModel(CSettingsStore* pSettingsStore, const CAlphInfo* pAlph,
                                       const CAlphabetMap* pAlphMap)
    : CLanguageModel(pAlph->iEnd - 1), m_pSettingsStore(pSettingsStore), m_pAlphInfo(pAlph), NodesAllocated(0),
...
void CWordLanguageModel::GetProbs(Context context, std::vector<unsigned int>& probs, int norm, int iUniform) const {
    ...
    for (int i(0); i < iNumSymbols; ++i) {
        dProbs[i] += wordcontext->m_dSpellingFactor * wordcontext->oSpellingProbs[i] /
                     static_cast<double>(wordcontext->m_iSpellingNorm);
        dNorm += dProbs[i];
    }
...
void CWordLanguageModel::CollapseContext(CWordLanguageModel::CWordContext& context, bool bLearn) {
    ...
    if (max_order == 0) {
        ...
        if (bLearn) { // Only do this if we are learning
            ...
            while ((pCurrent != NULL) && !bUpdateExclusion) {
                ...
                for (std::vector<symbol>::iterator it(oSymbols.begin()); it != oSymbols.end(); ++it) {
                    ...
                    if (pTmpChild == NULL) {
                        ...
                        if (pTmpChild->count == 0) bUpdateExclusion = false;
                        ++(pTmpChild->count);
                    }
            ...
            //      for( std::vector< std::vector< CWordnode* >* >::iterator it( oNodeCache.begin() ); it !=
            //      oNodeCache.end(); ++it ) {
            for (unsigned int i(0); i < oSymbols.size(); ++i) {

                CWordnode* pPreviousNode(NULL); // Start with a NULL pointer

                for (std::vector<CWordnode*>::reverse_iterator it2(apNodeCache[i]->rbegin());
                     it2 != apNodeCache[i]->rend(); ++it2) {
