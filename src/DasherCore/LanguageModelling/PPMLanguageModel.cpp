...
CAbstractPPM::CAbstractPPM(CSettingsStore* pSettingsStore, int iNumSyms, CPPMnode* pRoot, int iMaxOrder)
    : CLanguageModel(iNumSyms), m_pRoot(pRoot), m_pSettingsStore(pSettingsStore),
      m_iMaxOrder(iMaxOrder < 0 ? m_pSettingsStore->GetLongParameter(LP_LM_MAX_ORDER) : iMaxOrder),
      bUpdateExclusion(m_pSettingsStore->GetLongParameter(LP_LM_UPDATE_EXCLUSION) != 0), m_ContextAlloc(1024) {
...
bool CAbstractPPM::CPPMnode::eq(CAbstractPPM::CPPMnode* other, std::map<CPPMnode*, CPPMnode*>& equivs) {
    ...
    for (std::map<symbol, CPPMnode*>::iterator it1 = thisCh.begin(), it2 = otherCh.begin(); it1 != thisCh.end();
         it1++, it2++)
...
void CAbstractPPM::CPPMnode::AddChild(CPPMnode* pNewChild, int numSymbols) {
    if (m_iNumChildSlots < 0) {
        ...
        if (m_iNumChildSlots == 0) {
            ...
            while (m_ppChildren[idx %= m_iNumChildSlots])
                ++idx;
            // found NULL
            int stop = idx;
...
bool CPPMLanguageModel::RecursiveWrite(CPPMnode* pNode, CPPMnode* pNextSibling, std::map<CPPMnode*, int>* pmapIdx,
                                       int* pNextIdx, std::ofstream* pOutputFile) {
