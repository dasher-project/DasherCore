...
CDictLanguageModel::CDictLanguageModel(CSettingsStore* pSettingsStore, const CAlphInfo* pAlph,
    ...
    : CLanguageModel(pAlph->iEnd - 1), m_pSettingsStore(pSettingsStore), m_pAlph(pAlph), NodesAllocated(0),
      max_order(0), m_NodeAlloc(8192), m_ContextAlloc(1024) {
...
void CDictLanguageModel::GetProbs(Context context, std::vector<unsigned int>& probs, int norm, int iUniform) const {
    ...
    while (pSymbol) {
        int sym = pSymbol->sbl;
        if (!(doExclusion && exclusions[sym])) iTotal += pSymbol->count;
    ...
    for (i = 0; i < iNumSymbols; i++)
        if (!(doExclusion && exclusions[i])) symbolsleft++;
...
// //   ulong p=(((size_of_slice/2)*(2*s->count-1))/ulong(total)); // Changed to multiply before divide to avoid
// underflow problems when counts are really high - however, there's a chance this will cause overflow problems
...
// //                   // free(exclusions); // !!!
// //                   // !!! NB by IAM: p577 Stroustrup 3rd Edition: "Allocating an object using new and deleting it
// using free() is asking for trouble"
// //   //              delete[] exclusions;
// //                   return true;
// }
