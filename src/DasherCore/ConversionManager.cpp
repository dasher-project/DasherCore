...
// Note the new implementation in Mandarin Dasher may not be compatible with the previous implementation of Japanese
// Dasher Need to reconcile (a small project)

using namespace Dasher;

CConversionManager::CConversionManager(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface,
...
CDasherScreen::Label* CConversionManager::GetLabel(const char* pszConversion) {
    ...
    if (m_vLabels[strConv]) return m_vLabels[strConv];
...
void CConversionManager::RecursiveDumpTree(std::ostream& out, SCENode* pCurrent, unsigned int iDepth) {
    ...
    for (std::vector<SCENode*>::const_iterator it = children.begin(); it != children.end(); it++) {
        ...
        for (unsigned int i(0); i < iDepth; ++i)
            out << "-";
        out << " " << pChild->pszConversion
            << std::endl; //" " << pChild->IsHeadAndCandNum << " " << pChild->CandIndex << " " << pChild->IsComplete <<
                          //" " << pChild->AcCharCount << std::endl;
        RecursiveDumpTree(out, pChild, iDepth + 1);
    }
}

void CConversionManager::CConvNode::GetContext(CDasherInterfaceBase* pInterface, const CAlphabetMap* pAlphabetMap,
                                               std::vector<symbol>& vContextSymbols, int iOffset, int iLength) {
    ...
    if (pCurrentSCENode) {
        ...
        if ((GetChildren())[0]->mgr() == m_pMgr) {
            if (static_cast<CConvNode*>(GetChildren()[0])->m_pMgr == m_pMgr) {
                m_pMgr->m_pInterface->editProtect(
                    this); // TODO used to pass in offset 0, will now get this node's offset...
            }
        }
    } else {
    ...
    if (pSCENode && !pSCENode->GetChildren().empty()) {
        ...
        for (std::vector<SCENode*>::const_iterator it = vChildren.begin(); it != vChildren.end(); it++) {
            ...
            if (pCurrentSCEChild->Symbol != -1)
                mgr()->m_pLanguageModel->EnterSymbol(pNewNode->iContext,
                                                     pCurrentSCEChild->Symbol); // TODO: Don't use symbols?
