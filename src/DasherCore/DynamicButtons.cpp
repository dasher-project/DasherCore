...
void CDynamicButtons::GetUISettings(std::vector<Dasher::Parameter>& List) {
    ...
    AddSettings(List, {BP_BACKOFF_BUTTON, BP_AUTO_SPEEDCONTROL, LP_DYNAMIC_SPEED_FREQ, LP_DYNAMIC_SPEED_INC,
                       LP_DYNAMIC_SPEED_DEC});
}

CDynamicButtons::CDynamicButtons(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface,
        ...
        if (m_pSettingsStore->GetBoolParameter(BP_AUTO_SPEEDCONTROL) && m_uSpeedControlTime < iTime) {
            if (m_uSpeedControlTime > 0) // has actually been set?
                m_pSettingsStore->SetLongParameter(
                    LP_MAX_BITRATE,
                    static_cast<long>(m_pSettingsStore->GetLongParameter(LP_MAX_BITRATE) *
                                      (1.0 + m_pSettingsStore->GetLongParameter(LP_DYNAMIC_SPEED_INC) / 100.0)));
...
void CDynamicButtons::KeyDown(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                              CDasherModel* pModel) {

    if (((Key == Keys::Big_Start_Stop_Key) || (Key == Keys::Button_1) || (Key == Keys::Primary_Input)) &&
        !m_pSettingsStore->GetBoolParameter(BP_BACKOFF_BUTTON))
        return;
...
void CDynamicButtons::KeyUp(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                            CDasherModel* pModel) {
