...
void Dasher::CSmoothingFilter::GetUISettings(std::vector<Dasher::Parameter>& List) {
    ...
    AddSettings(List, {LP_SMOOTH_TAU, BP_SMOOTH_DRAW_MOUSE_LINE, BP_SMOOTH_DRAW_MOUSE, BP_SMOOTH_ONLY_FORWARD,
                       BP_SMOOTH_PRESS_MODE});
}

Dasher::CSmoothingFilter::CSmoothingFilter(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface,
                                           CFrameRate* pFramerate, const char* szName)
    : CPressFilter(pSettingsStore, pInterface, pFramerate, szName) {}

void Dasher::CSmoothingFilter::KeyDown(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pDasherView,
...
void Dasher::CSmoothingFilter::KeyUp(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pView,
                                     CDasherInput* pInput, CDasherModel* pModel) {
    if (m_pSettingsStore->GetBoolParameter(BP_SMOOTH_PRESS_MODE)) {
...
void Dasher::CSmoothingFilter::Timer(unsigned long Time, CDasherView* pView, CDasherInput* pInput, CDasherModel* pModel,
    ...
    if (currentlySmoothing) {
        const float smoothingAlpha =
            std::max(std::min(1.0f - std::exp(-static_cast<float>(Time - LastUpdate) /
                                              static_cast<float>(m_pSettingsStore->GetLongParameter(LP_SMOOTH_TAU))),
                              1.0f),
                     0.0f);
    ...
    LastUpdate = Time;
    ExecuteMovement(Time, pView, pModel, static_cast<screenint>(SmoothedPositionX),
                    static_cast<screenint>(SmoothedPositionY));
}
...
bool Dasher::CSmoothingFilter::DecorateView(CDasherView* pView, CDasherInput* pInput) {
    ...
    if (currentlySmoothing && m_pSettingsStore->GetBoolParameter(BP_SMOOTH_DRAW_MOUSE)) {
        ...
        pView->DasherDrawCentredRectangle(
            lastCursorPosX, lastCursorPosY, 5,
            pView->GetNamedColor(NamedColor::inputPosition).lerp(ColorPalette::white, 0.75f), ColorPalette::noColor,
            false);
        result = true;
    }
