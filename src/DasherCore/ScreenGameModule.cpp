...
CScreenGameModule::CScreenGameModule(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pIntf, CDasherView* pView,
                                     CDasherModel* pModel)
...
void CScreenGameModule::DrawText(CDasherView* pView) {
    ...
    if (!m_pLabTarget || !m_pLabEntered) {
        ...
        for (bool bIncreased = false;;) {
            ...
            if (w < maxX / 3 && m_iFirstSym) {
                ...
                bIncreased = true;
            } else if (w > 2 * maxX / 3 && m_iFirstSym < static_cast<int>(targetSyms().size()) - 1 && !bIncreased)
        ...
        for (bool bIncreased = false;;) {
            ...
            if (w < 2 * maxX / 3 && m_iLastSym < static_cast<int>(targetSyms().size()) - 1) {
                m_strTarget += m_pAlph->GetText(targetSyms()[m_iLastSym++]);
                bIncreased = true;
            } else if (w > maxX && m_iLastSym && !bIncreased)
                m_strTarget =
                    m_strTarget.substr(0, m_strTarget.length() - m_pAlph->GetText(targetSyms()[--m_iLastSym]).length());
            else
                break;
            // m_strTarget changed
            delete m_pLabTarget;
    ...
    pScreen->DrawRectangle(
        0, 0, rectDims.first, rectDims.second,
        pView->GetNamedColor((m_pLabWrong) ? NamedColor::warningTextBackground : NamedColor::infoTextBackground),
        pView->GetNamedColor(NamedColor::infoText), 1);
