...
class CSimplePooledAlloc {
  ...
  private:
    class CPool {
      public:
        CPool(std::size_t iSize) : m_iCurrent(0), m_iSize(iSize) { m_pData = new T[m_iSize]; }
        ~CPool() { delete[] m_pData; }
        T* Alloc() const {
...
template <typename T>
CSimplePooledAlloc<T>::CSimplePooledAlloc(std::size_t iSize) : m_iBlockSize(iSize), m_iCurrent(0) {
