...
CDashIntfScreenMsgs::CDashIntfScreenMsgs(CSettingsStore* pSettingsStore) : CDashIntfSettings(pSettingsStore) {}
...
bool CDashIntfScreenMsgs::FinishRender(unsigned long ulTime) {
    ...
    while (!m_dqAsyncMessages.empty() && m_dqAsyncMessages.front().second &&
           ulTime - m_dqAsyncMessages.front().second > static_cast<unsigned long>(GetLongParameter(LP_MESSAGE_TIME))) {
    ...
    if (!m_dqAsyncMessages.empty() || !m_dqModalMessages.empty()) {
        ...
        for (std::deque<std::pair<CDasherScreen::Label*, unsigned long>>::iterator it = m_dqAsyncMessages.begin();
             it != m_dqAsyncMessages.end() && iY > iMinY; it++) {
            if (it->second == 0) {
        ...
        if (!m_dqModalMessages.empty()) {
            ...
            for (std::deque<std::pair<CDasherScreen::Label*, unsigned long>>::iterator it = m_dqModalMessages.begin();
                 it != m_dqModalMessages.end() && iY > iMinY; it++) {
                if (bDisp) {
        ...
        for (std::deque<std::pair<CDasherScreen::Label*, unsigned long>>::const_iterator it = m_dqAsyncMessages.begin();
             it != m_dqAsyncMessages.end(); it++) {
            if (it->second == 0) continue;
            std::pair<screenint, screenint> textDims =
                pScreen->TextSize(it->first, GetLongParameter(LP_MESSAGE_FONTSIZE));
            // black (5) rectangle:
            pScreen->DrawRectangle((iSW - textDims.first) / 2, iY, (iSW + textDims.first) / 2, iY + textDims.second,
                                   GetView()->GetNamedColor(NamedColor::infoTextBackground), ColorPalette::noColor, -1);
            // white (0) text for non-modal
            pScreen->DrawString(it->first, (iSW - textDims.first) / 2, iY, GetLongParameter(LP_MESSAGE_FONTSIZE),
                                GetView()->GetNamedColor(NamedColor::infoText));
            iY += textDims.second;
        }

        for (std::deque<std::pair<CDasherScreen::Label*, unsigned long>>::const_iterator it = m_dqModalMessages.begin();
             it != m_dqModalMessages.end(); it++) {
            if (it->second == 0) continue;
            std::pair<screenint, screenint> textDims =
                pScreen->TextSize(it->first, GetLongParameter(LP_MESSAGE_FONTSIZE));
            // black (5) rectangle:
            pScreen->DrawRectangle((iSW - textDims.first) / 2, iY, (iSW + textDims.first) / 2, iY + textDims.second,
                                   GetView()->GetNamedColor(NamedColor::warningTextBackground), ColorPalette::noColor,
                                   -1);
            // yellow (111) for modal
            pScreen->DrawString(it->first, (iSW - textDims.first) / 2, iY, GetLongParameter(LP_MESSAGE_FONTSIZE),
                                GetView()->GetNamedColor(NamedColor::warningText));
            iY += textDims.second;
        }
    }
    return bMsgsChanged;
