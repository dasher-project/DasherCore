...
void CDasherView::DasherSpaceLine(myint x1, myint y1, myint x2, myint y2, int iWidth,
    ...
    for (int i = static_cast<int>(vPoints.size()); i-- > 0;)
        pts[i] = vPoints[i];
...
void CDasherView::DasherPolyarrow(myint* x, myint* y, int n, int iWidth, const ColorPalette::Color& color,
                                  double dArrowSizeFactor) const {
...
void CDasherView::DasherDrawRectangle(myint iDasherMaxX, myint iDasherMinY, myint iDasherMinX, myint iDasherMaxY,
    ...
    Screen()->DrawRectangle(std::min(iScreenLeft, iScreenRight), std::min(iScreenTop, iScreenBottom),
                            std::max(iScreenLeft, iScreenRight), std::max(iScreenTop, iScreenBottom), color,
                            outlineColor, iThickness);
}

/// Draw a rectangle centred on a given dasher co-ordinate, but with a size specified in screen co-ordinates (used for
/// drawing the mouse blob)

void CDasherView::DasherDrawCentredRectangle(myint iDasherX, myint iDasherY, screenint iSize,
                                             const ColorPalette::Color& color, const ColorPalette::Color& outlineColor,
                                             bool bDrawOutline) const {
    screenint iScreenX;
    screenint iScreenY;

    Dasher2Screen(iDasherX, iDasherY, iScreenX, iScreenY);

    Screen()->DrawRectangle(iScreenX - iSize, iScreenY - iSize, iScreenX + iSize, iScreenY + iSize, color, outlineColor,
                            bDrawOutline ? 1 : 0);
}
