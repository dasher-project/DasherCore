...
static SModuleSettings sSettings[] = {
    ...
    /* TRANSLATORS: The boxes (zoom targets) in button mode can either be the same size, or different sizes - this is
       the extent to which the sizes are allowed to differ from each other. */
    /* XXX PRLW: 128 log(2) = 89, where 2 is the ratio of adjacent boxes
     * however the code seems to use ratio = (129/127)^-r, instead of
     * ratio = exp(r/128) used in the design document
     */
    {LP_R, T_LONG, -89, 89, 1, 10, _("Box non-uniformity")}
    /* TRANSLATORS: Intercept keyboard events for 'special' keys even when the Dasher window doesn't have keyboard
       focus.*/
...
CButtonMode::CButtonMode(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface, bool bMenu,
                         const char* szName)
...
bool CButtonMode::DecorateView(CDasherView* pView, CDasherInput* pInput) {
    for (int i(0); i < m_iNumBoxes; ++i) {
        if (i != iActiveBox) NewDrawGoTo(pView, m_pBoxes[i].iDisplayTop, m_pBoxes[i].iDisplayBottom, false);
    }
...
void CButtonMode::Timer(unsigned long Time, CDasherView* pView, CDasherInput* pInput, CDasherModel* pModel,
                        CExpansionPolicy** pol) {
    bool m_bOldHighlight(m_bHighlight);
    m_bHighlight = (Time - m_iLastTime < 200);

    if (m_bOldHighlight != m_bHighlight) m_bDecorationChanged = true;

    CDasherButtons::Timer(Time, pView, pInput, pModel, pol);
}

void CButtonMode::KeyDown(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                          CDasherModel* pModel) {
    if (Key == Keys::Primary_Input) {
        ...
        if (m_bMenu) {
            ...
            for (int i = 0; i < m_iNumBoxes; i++) {
                if (iDasherY < m_pBoxes[i].iDisplayBottom && iDasherY > m_pBoxes[i].iDisplayTop &&
                    ...
                    CDasherButtons::KeyDown(iTime,
                                            (i == m_iNumBoxes - 1)
                                                ? Keys::Button_1
                                                : static_cast<Keys::VirtualKey>(
                                                      i + 2), // NOLINT(clang-analyzer-optin.core.EnumCastOutOfRange)
                                            pView, pInput, pModel);
                    return;
                }
            }
