...
static SModuleSettings gameSets[] = {
    ...
    {BP_GAME_HELP_DRAW_PATH, T_BOOL, -1, -1, -1, -1,
     _("When we give help, show the shortest path to the target sentence?")},
};

CGameModule::CGameModule(CSettingsStore* pSettingsStore, Dasher::CDasherInterfaceBase* pInterface, CDasherView* pView,
                         CDasherModel* pModel)
    ...
    m_pInterface->OnEditEvent.Subscribe(this, [this](CEditEvent::EditEventType type, const std::string& strText,
                                                     CDasherNode* node) { HandleEditEvent(type, strText, node); });

    m_pModel->OnNodeChildrenCreated.Subscribe(this, [this](CDasherNode* node) { HandleNodePopulated(node); });
}
...
void CGameModule::HandleNodePopulated(CDasherNode* pNode) {
    if (pNode->GetFlag(CDasherNode::NF_GAME) // if on game path, look for next/child node on path...
        && pNode->offset() + 1 < static_cast<int>(m_vTargetSymbols.size()))
...
void CGameModule::HandleViewChange(CDasherView* pView) {
    ...
    m_pView->OnViewChanged.Subscribe(this, [this](CDasherView* newView) { HandleViewChange(newView); });
    m_pView->OnGameNodeDraw.Subscribe(
        this, [this](CDasherNode* node, myint y1, myint y2) { HandleGameNodeDraw(node, y1, y2); });
}
...
void CGameModule::DecorateView(unsigned long lTime, CDasherView* pView, CDasherModel* pModel) {
    ...
    if (m_dSentenceStartNats == std::numeric_limits<double>::max()) m_dSentenceStartNats = pModel->GetNats();
...
void CGameModule::DrawBrachistochrone(CDasherView* pView) {
    ...
    pView->DasherSpaceArc(CenterY, llabs(CenterY - m_iTargetY), CDasherModel::ORIGIN_X, CDasherModel::ORIGIN_Y, 0,
                          m_iTargetY, pView->GetNamedColor(NamedColor::gameGuide),
                          2 * (int)m_pSettingsStore->GetLongParameter(LP_LINE_WIDTH));
}
...
void CGameModule::DrawHelperArrow(Dasher::CDasherView* pView) {
    ...
    //...then plot it.
    pView->DasherPolyarrow(iX, iY, noOfPoints, m_pSettingsStore->GetLongParameter(LP_LINE_WIDTH) * 4,
                           pView->GetNamedColor(NamedColor::gameGuide), 1.414);
}
