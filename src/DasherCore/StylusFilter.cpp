...
CStylusFilter::CStylusFilter(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface, CFrameRate* pFramerate,
                             const char* szName)
    : CDefaultFilter(pSettingsStore, pInterface, pFramerate, szName), m_pModel(NULL) {}

void CStylusFilter::KeyDown(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                            CDasherModel* pModel) {
...
void CStylusFilter::KeyUp(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                          CDasherModel* pModel) {
    if (Key == Keys::Primary_Input) {
        ...
        if (iTime - m_iKeyDownTime < static_cast<unsigned long>(m_pSettingsStore->GetLongParameter(LP_TAP_TIME))) {
            ...
            (m_pModel = pModel)
                ->ScheduleZoom(m_iLastY - m_iLastX, m_iLastY + m_iLastX,
                               m_pSettingsStore->GetLongParameter(LP_ZOOMSTEPS));
        } else {
            m_pInterface->Done();
        }
    } else
...
void CStylusFilter::ApplyClickTransform(myint& iDasherX, myint& iDasherY, CDasherView* pView) {
    AdjustZoomX(iDasherX, pView, m_pSettingsStore->GetLongParameter(LP_S),
                m_pSettingsStore->GetLongParameter(LP_MAXZOOM));
}
