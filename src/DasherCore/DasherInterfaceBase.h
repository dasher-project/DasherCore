...
class Dasher::CDasherInterfaceBase : public CMessageDisplay, private NoClones {
  public:
    ...
    virtual unsigned int ctrlOffsetAfterMove(unsigned int offsetBefore, bool bForwards, EditDistance iDist) {
        return offsetBefore;
    }
    ...
    // std::map<int, std::string>& GetAlphabets(); // map<key, value> int is a UID string can change. Store UID in
    // preferences. Display string to user. std::vector<std::string>& GetAlphabets(); std::vector<std::string>&
    // GetLangModels(); std::vector<std::string>& GetViews();
    ...
    void ScheduleRedraw() { m_bRedrawScheduled = true; };
    ...
    CGameModule* GetGameModule() { return m_pGameModule.get(); }
