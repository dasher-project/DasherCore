...
bool CClickFilter::DecorateView(CDasherView* pView, CDasherInput* pInput) {
    ...
    if (m_pSettingsStore->GetBoolParameter(BP_DRAW_MOUSE_LINE)) {
        ...
        AdjustZoomX(mouseX, pView, m_pSettingsStore->GetLongParameter(LP_S),
        ...
        if (m_pSettingsStore->GetBoolParameter(BP_CURVE_MOUSE_LINE)) {
            ...
            pView->DasherSpaceLine(x[1], y[1], x[2], y[2], m_pSettingsStore->GetLongParameter(LP_LINE_WIDTH),
                                   pView->GetNamedColor(NamedColor::inputLine));
        } else {
            ...
            //   y[2] = iDasherMaxY;
            // }
            pView->DasherPolyline(x, y, 3, m_pSettingsStore->GetLongParameter(LP_LINE_WIDTH),
                                  pView->GetNamedColor(NamedColor::inputLine));
        }
    }
    return bChanged;
...
void CClickFilter::KeyDown(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                           CDasherModel* pModel) {
    switch (Key) {
