...
void CDemoFilter::GetUISettings(std::vector<Dasher::Parameter>& List) {
    ...
    AddSettings(List, {BP_DRAW_MOUSE, BP_DRAW_MOUSE, BP_START_MOUSE, LP_MAX_BITRATE, LP_FRAMERATE, LP_FRAMERATE,
                       LP_LINE_WIDTH, LP_DEMO_SPRING, LP_DEMO_NOISE_MEM, LP_DEMO_NOISE_MAG});
}

CDemoFilter::CDemoFilter(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface, CFrameRate* pFramerate)
    : CDynamicFilter(pSettingsStore, pInterface, pFramerate, _("Demo Mode (no input)")), m_dNoiseX(0.0), m_dNoiseY(0.0),
      m_iDemoX(0), m_iDemoY(0) {}
...
bool CDemoFilter::DecorateView(CDasherView* pView, CDasherInput* pInput) {
    ...
    if (m_pSettingsStore->GetBoolParameter(BP_DRAW_MOUSE)) {
        pView->DasherDrawCentredRectangle(m_iDemoX, m_iDemoY, 5, pView->GetNamedColor(NamedColor::inputPosition),
...
{
    double u1 = (double(rand()) + 1) / (double(RAND_MAX) + 1.0); // NOLINT(cert-msc30-c,cert-msc50-cpp)
    double u2 = (double(rand()) + 1) / (double(RAND_MAX) + 1.0); // NOLINT(cert-msc30-c,cert-msc50-cpp)
...
void CDemoFilter::Timer(unsigned long Time, CDasherView* m_pDasherView, CDasherInput* pInput,
                        CDasherModel* m_pDasherModel, CExpansionPolicy** pol) {
    if (isPaused()) return;
    ...
    iIdealUnitVec[0] = double(CDasherModel::ORIGIN_Y < iTargetY ? (iCenterY - CDasherModel::ORIGIN_Y)
                                                                : (CDasherModel::ORIGIN_Y - iCenterY));
...
void CDemoFilter::KeyDown(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pDasherView, CDasherInput* pInput,
                          CDasherModel* pModel) {
...
void CDemoFilter::HandleEvent(Parameter parameter) {
    ...
    case LP_FRAMERATE: {
        ...
        double lambda = 0.7 * m_pSettingsStore->GetLongParameter(LP_MAX_BITRATE) /
                        (double)m_pSettingsStore->GetLongParameter(LP_FRAMERATE);
