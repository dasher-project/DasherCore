...
void COneButtonFilter::Timer(unsigned long Time, CDasherView* pView, CDasherInput* pInput, CDasherModel* m_pDasherModel,
...
void COneButtonFilter::KeyDown(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                               CDasherModel* pModel) {
    if (bStarted) {
        iLocation = std::max(iTime - iStartTime - m_pSettingsStore->GetLongParameter(LP_DYNAMIC_BUTTON_LAG), 0ul) *
                    4096 / m_pSettingsStore->GetLongParameter(LP_STATIC1B_TIME);
        if (iLocation > 8192) {
