...
static UserLogParamMask s_UserLogParamMaskTable[] = {
    ...
    {LP_MAX_BITRATE, userLogParamOutputToSimple | userLogParamTrackMultiple | userLogParamTrackInTrial |
                         userLogParamForceInTrial | userLogParamShortInCycle},
...
CUserLog::CUserLog(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface, int iLogTypeMask)
    ...
    if (m_pApplicationSpan == NULL)
        m_pInterface->GetGlobalApplicationLogger()->LogNormal(
            "CUserLog::CUserLog, failed to create m_pApplicationSpan!");
...
CUserLog::~CUserLog() {
    ...
    if ((m_bSimple) && (m_pSimpleLogger != NULL)) m_pSimpleLogger->LogDebug("stop");
...
void CUserLog::InitUsingMask(int iLogLevelMask) {
    ...
    if (iLogLevelMask & userLogSimple) {
        ...
        if (m_pSimpleLogger == NULL)
            m_pSimpleLogger =
                new CFileLogger(USER_LOG_SIMPLE_FILENAME, eLogLevel::logDEBUG, logTimeStamp | logDateStamp);
    }

    if (iLogLevelMask & userLogDetailed) m_bDetailed = true;
}
...
void CUserLog::StartWriting() {
    ...
    if (m_bSimple) {
        ...
        if ((m_bNeedToWriteCanvas) && (m_pSimpleLogger != NULL)) {
            m_pSimpleLogger->LogDebug("canvas:\t%d\t%d\t%d\t%d", m_sCanvasCoordinates.top, m_sCanvasCoordinates.left,
                                      m_sCanvasCoordinates.bottom, m_sCanvasCoordinates.right);
    ...
    if (m_bDetailed) {
        ...
        if (pTrial == NULL) pTrial = AddTrial();

        if (pTrial != NULL)
            pTrial->StartWriting();
        else
            m_pInterface->GetGlobalApplicationLogger()->LogNormal(
                "CUserLog::StartWriting, failed to create new pTrial!");
    }
...
void CUserLog::StopWriting() {
    ...
    if (m_bIsWriting) {
        ...
        if ((m_bSimple) && (m_pSimpleLogger != NULL)) m_pSimpleLogger->LogDebug("%s", GetStartStopCycleStats().c_str());
...
void CUserLog::NewTrial() {
    ...
    if (USER_LOG_DUMP_AFTER_TRIAL) WriteXML();
...
void CUserLog::AddParam(const std::string& strName, const std::string& strValue, int iOptionMask) {
    ...
    if ((bOutputToSimple) && (m_bSimple) && (m_pSimpleLogger != NULL) && (m_bInitIsDone) && (!bShortInCycle)) {
    ...
    if (pNewParam == NULL) {
        m_pInterface->GetGlobalApplicationLogger()->LogNormal(
            "CUserLog::AddParam, failed to create CUserLogParam object!");
        return;
    }
    ...
    if (bTrackMultiple) pNewParam->strTimeStamp = CTimeSpan::GetTimeStamp();
    ...
    if ((bTrackInTrial) && (m_bDetailed)) {
        ...
        if (pTrial != NULL) pTrial->AddParam(strName, strValue, iOptionMask);
    }
}
...
void CUserLog::AddMouseLocation(int iX, int iY, float dNats) {
    ...
    if (UpdateMouseLocation()) {
        if (m_bDetailed) {
            ...
            if (pTrial->IsWriting()) pTrial->AddMouseLocation(iX, iY, dNats);
        }

        // Keep track of the dNats for the current mouse position
        if (m_bIsWriting) m_dCycleNats = dNats;
    }
}
...
void CUserLog::AddCanvasSize(int iTop, int iLeft, int iBottom, int iRight) {
    ...
    if ((m_bSimple) && (m_pSimpleLogger != NULL) &&
        ((m_sCanvasCoordinates.top != iTop) || (m_sCanvasCoordinates.left != iLeft) ||
         (m_sCanvasCoordinates.bottom != iBottom) || (m_sCanvasCoordinates.right != iRight)))
...
void CUserLog::AddMouseLocationNormalized(int iX, int iY, bool bStoreIntegerRep, float dNats) {
    ...
    // Check to see if it is time to actually push a mouse location update
    if (UpdateMouseLocation()) {
        if ((m_sCanvasCoordinates.bottom == 0) && (m_sCanvasCoordinates.left == 0) &&
            (m_sCanvasCoordinates.right == 0) && (m_sCanvasCoordinates.top == 0)) {
            m_pInterface->GetGlobalApplicationLogger()->LogNormal(
                "CUserLog::AddMouseLocationNormalized, called before AddCanvasSize()?");
            return;
        }
        ...
        if (m_bDetailed) {
            ...
            // Only record mouse locations during navigation
            if (pTrial->IsWriting()) pTrial->AddMouseLocationNormalized(iX, iY, bStoreIntegerRep, dNats);
        }
...
void CUserLog::KeyDown(Dasher::Keys::VirtualKey Key, int iType, int iEffect) {
    ...
    if (pTrial) pTrial->AddKeyDown(Key, iType, iEffect);
}
...
std::string CUserLog::GetXML() {
    ...
    strResult += "<UserLog>\n";
    if (m_pApplicationSpan != NULL) strResult += m_pApplicationSpan->GetXML("\t");
...
CUserLogTrial* CUserLog::GetCurrentTrial() {
    ...
    if (m_vpTrials.size() <= 0) return NULL;
...
CUserLogTrial* CUserLog::AddTrial() {
    ...
    if (pTrial != NULL) {
        ...
        m_pInterface->GetGlobalApplicationLogger()->LogNormal(
            "CUserLog::AddTrial, failed to create CUserLogTrialSpeech!");
...
std::string CUserLog::GetStartStopCycleStats() {
    ...
    if (m_pCycleTimer == NULL) {
        m_pInterface->GetGlobalApplicationLogger()->LogNormal(
            "CUserLog::GetStartStopCycleStats, cycle timer was NULL!");
        return "";
    }
    ...
    snprintf(m_szTempBuffer, TEMP_BUFFER_SIZE, "tsbdxym:\t%0.3f\t%zu\t%0.6f\t%d\t%0.3f\t%0.3f%s",
             m_pCycleTimer->GetElapsed(), m_vCycleHistory.size(), GetCycleBits(), m_iCycleNumDeletes, dNormX, dNormY,
...
void CUserLog::ComputeSimpleMousePos(int iX, int iY) {
    ...
    if ((m_bSimple) && (m_bIsWriting)) {
        ...
        m_dCycleMouseNormXSum +=
            CUserLocation::ComputeNormalizedX(iX, m_sCanvasCoordinates.left, m_sCanvasCoordinates.right);

        m_dCycleMouseNormYSum +=
            CUserLocation::ComputeNormalizedY(iY, m_sCanvasCoordinates.top, m_sCanvasCoordinates.bottom);
        m_iCycleMouseCount++;
    }
}
...
void CUserLog::PrepareNewTrial() {
    ...
    if (pTrial != NULL) {
        ...
        pTrial->AddCanvasSize(m_sCanvasCoordinates.top, m_sCanvasCoordinates.left, m_sCanvasCoordinates.bottom,
                              m_sCanvasCoordinates.right);

        pTrial->AddWindowSize(m_sWindowCoordinates.top, m_sWindowCoordinates.left, m_sWindowCoordinates.bottom,
                              m_sWindowCoordinates.right);

    } else
        m_pInterface->GetGlobalApplicationLogger()->LogNormal(
            "CUserLog::PrepareNewTrial, failed to create CUserLogTrial");
}
...
std::string CUserLog::GetCycleParamStats() {
    ...
    if (m_vParams.size() <= 0) return strResult;
...
void CUserLog::UpdateParam(Parameter parameter, int iOptionMask) {
    ...
    default: {
        m_pInterface->GetGlobalApplicationLogger()->LogNormal(
            "CUserLog::UpdateParam, matched parameter %d but unknown type %d", parameter, GetParameterType(parameter));
        break;
    }
    };
}