...
static SModuleSettings sSettings[] = {
    ...
    {LP_TWO_PUSH_LONG, T_LONG, 128, 1024, 2048 /*divisor*/, 128 /*step*/,
     _("Distance between down markers (long gap)")},
    ...
    {BP_TWO_PUSH_RELEASE_TIME, T_BOOL, -1, -1, -1, -1,
     _("Use push and release times of single press rather than push times of two presses")},
    /* TRANSLATORS: Backoff = reversing in Dasher to correct mistakes. This allows a single button to be dedicated to
       activating backoff, rather than using multiple presses of other buttons, and another to be dedicated to starting
       and stopping. 'Button' in this context is a physical hardware device, not a UI element.*/
...
long CTwoPushDynamicFilter::upDist() {
    return m_pSettingsStore->GetLongParameter(LP_TWO_PUSH_OUTER) -
           (m_pSettingsStore->GetLongParameter(LP_TWO_PUSH_LONG) *
            m_pSettingsStore->GetLongParameter(LP_TWO_PUSH_SHORT)) /
               100;
}
...
bool CTwoPushDynamicFilter::DecorateView(CDasherView* pView, CDasherInput* pInput) {
    if (isRunning()) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            pScreen->DrawRectangle(x1, y1, x2, y2, pView->GetNamedColor(NamedColor::twoPushDynamicOuterGuides),
                                   ColorPalette::noColor, 0);
        }
    }
    ...
    if (m_bDecorationChanged && isRunning() && m_dNatsSinceFirstPush > -std::numeric_limits<double>::infinity()) {
        for (int i = 0; i < 2; i++) {
            GuideLine(pView, m_aiMarker[i],
                      (i == m_iActiveMarker) ? NamedColor::twoPushDynamicActiveMarker
                                             : NamedColor::twoPushDynamicInactiveMarker);
        }
    }
...
void CTwoPushDynamicFilter::KeyUp(unsigned long Time, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                                  CDasherModel* pModel) {
    if (Key == Keys::Primary_Input && !m_pSettingsStore->GetBoolParameter(BP_BACKOFF_BUTTON))
        // mouse click - will be ignored by superclass method.
        // simulate press of button 2...
        Key = Keys::Button_2;
    if (m_pSettingsStore->GetBoolParameter(BP_TWO_PUSH_RELEASE_TIME) && isRunning() && Key == m_iHeldId &&
        m_dNatsSinceFirstPush != -std::numeric_limits<double>::infinity())
...
void CTwoPushDynamicFilter::TimerImpl(unsigned long iTime, CDasherView* m_pDasherView, CDasherModel* m_pDasherModel,
    ...
    if (m_dNatsSinceFirstPush > -std::numeric_limits<double>::infinity()) {
        ...
        double dLogGrowth(m_pDasherModel->GetNats() - m_dNatsSinceFirstPush),
            dOuter(m_pSettingsStore->GetLongParameter(LP_TWO_PUSH_OUTER)), dUp(upDist()), dDown(downDist());
        ...
        if (dLogGrowth > m_dMaxLongTwoPushTime) {
            ...
            m_bDecorationChanged |= doSet(m_iActiveMarker, -1 /*in middle (neither/both) or too short*/);
    }
