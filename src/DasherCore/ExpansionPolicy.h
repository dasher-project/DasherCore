...
class CExpansionPolicy {
  public:
    ...
    /// dMaxCost should be the value returned by pushNode from the call for the node most closely enclosing pNode (that
    /// was pushed!) for the first (outermost) node, i.e. when no enclosing node has been passed, (+ive) INFINITY should
    /// be passed in.
