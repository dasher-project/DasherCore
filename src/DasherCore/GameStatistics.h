...
U nthMoment(int n, const std::vector<T>& v, U mean) {
    return std::accumulate(v.begin(), v.end(), U(),
                           [mean, n](const U& sum, const T& current) { return sum + std::pow(current - mean, n); }) /
           v.size();
}
...
#define SEP " "
    m_Statsbreakdown << "Samples: " << v.size() << SEP << "Mean: " << m1 << SEP << "StdDev: " << dev << SEP
                     << "Skew: " << skew << SEP << "Kurt: " << kurt << SEP;
#undef SEP
