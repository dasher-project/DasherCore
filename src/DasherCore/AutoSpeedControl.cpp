...
CAutoSpeedControl::CAutoSpeedControl(CSettingsStore* pSettingsStore) : m_pSettingsStore(pSettingsStore) {
...
void CAutoSpeedControl::SpeedControl(myint iDasherX, myint iDasherY, CDasherView* pView) {
    if (m_pSettingsStore->GetBoolParameter(BP_AUTO_SPEEDCONTROL)) {
        ...
        m_dBitrate =
            m_pSettingsStore->GetLongParameter(LP_MAX_BITRATE) / 100.0; //  stored as long(round(true bitrate * 100))
