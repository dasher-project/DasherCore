...
class CAlphBase : public CDasherNode {
  public:
    ...
    ///  \param pParent parent of the symbol node to create; could be the previous root, or an intervening node (e.g.
    ///  group)
    ...
    ///  would contain this node (see IsInGroup). Subclasses can override to graft themselves into the hierarchy, if
    ///  appropriate.
    ///  \param pParent parent of the symbol node to create; could be the previous root, or an intervening node (e.g.
    ///  group)
  ...
  protected:
    ...
    virtual bool UseAltColor() const { return (offset() & 1) == 0; }
...
class CSymbolNode : public CAlphNode {
  public:
    ...
    bool UseAltColor() const override { return (offset() & 1) == 0; }
...
class CGroupNode : public CAlphNode {
  public:
    ...
    /// Override: if the group to create is the same as this node's group, return this node instead of creating a new
    /// one
...
class CAlphabetManager : public CNodeManager {
  ...
  public:
    ...
    CAlphabetManager(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface, CNodeCreationManager* pNCManager,
                     const CAlphInfo* pAlphabet);
  ...
  protected:
    ...
    std::pair<symbol, CLanguageModel::Context> GetContextSymbols(CDasherNode* pParent, int iRootOffset,
