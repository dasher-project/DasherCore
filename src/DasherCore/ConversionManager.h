...
class CConversionManager : public CNodeManager {
  ...
  public:
    CConversionManager(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface,
                       CNodeCreationManager* pNCManager, const CAlphInfo* pAlphabet, CLanguageModel* pLanguageModel);
  ...
  protected:
    ...
    /// @param normalization Normalisation constant for the child sizes (TODO: check that this is a sensible value, ie
    /// the same as Dasher normalisation).
    /// @param uniform Unsure - document this.
    ///
    virtual void AssignSizes(const std::vector<SCENode*>& vChildren, Dasher::CLanguageModel::Context context,
                             long normalization, int uniform) = 0;
