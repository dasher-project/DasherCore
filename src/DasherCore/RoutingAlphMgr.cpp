...
CRoutingAlphMgr::CRoutingAlphMgr(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface,
...
void CRoutingAlphMgr::InitMap() {
    m_vBaseSyms.reserve(m_pAlphabet->iEnd);
    m_vBaseSyms.push_back(0);                // base for unknown route = unknown!
    ...
    while (!vGroups.empty()) {
        const SGroupInfo* g(vGroups.back());
        vGroups.pop_back();
        if (!g) continue;
        for (int i = g->iStart; i < g->iEnd; i++)
            m_vGroupsByRoute[i] = g;
...
void CRoutingAlphMgr::CreateLanguageModel() {
    m_pLanguageModel = new CRoutingPPMLanguageModel(m_pSettingsStore, &m_vBaseSyms, &m_vRoutes,
                                                    m_pAlphabet->m_iConversionID == CAlphInfo::RoutingContextSensitive);
}
...
std::string CRoutingAlphMgr::CRoutedSym::trainText() {
    ...
    if (routes.size() != 1)
        if (const SGroupInfo* g = mgr()->m_vGroupsByRoute[iSymbol])
            return mgr()->m_pAlphabet->m_strConversionTrainStart + g->strName +
                   mgr()->m_pAlphabet->m_strConversionTrainStop + t;
    return t;
}

CRoutingAlphMgr::CRoutedSym::CRoutedSym(int iOffset, CDasherScreen::Label* pLabel, CRoutingAlphMgr* pMgr,
                                        symbol iSymbol)
...
symbol CRoutingAlphMgr::CRoutingTrainer::getRoute(bool bHaveRoute, const std::string& strRoute, symbol baseSym) {
    ...
    for (std::set<symbol>::iterator it = candidates.begin(); it != candidates.end(); it++)
        if (const SGroupInfo* g = m_pMgr->m_vGroupsByRoute[*it])
            if (g->strName == strRoute) named.insert(*it);
    ...
    if (bHaveRoute) {
        m_pMsgs->FormatMessage(
            (named.size() == 0)
                ? "Warning: training file contains character '%s' as member of group '%s', but no group of that name "
                  "contains the character. Ignoring group specifier."
            ...
            m_pInfo->GetDisplayText(baseSym).c_str(), strRoute.c_str());
    }
    ...
    for (symbol sym; (sym = syms.next(m_pAlphabet)) != -1;) {
        if (sym == m_iStartSym) {
            if (sym != 0 || syms.peekBack() == m_pInfo->m_strConversionTrainStart) {
                ...
                strRoute.clear();
                bHaveRoute = true;
                for (std::string s; (s = syms.peekAhead(m_pAlphabet)).length(); strRoute += s) {
        ...
        if (readEscape(trainContext, sym, syms))
