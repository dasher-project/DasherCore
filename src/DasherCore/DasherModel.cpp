...
bool CDasherModel::NextScheduledStep() {
    ...
    while (newRootmax >= m_Rootmax_max || newRootmin <= m_Rootmin_min) {
        ...
        if (m_Root->GetChildren().empty()) return false;
        for (CDasherNode::ChildMap::const_iterator it = m_Root->GetChildren().begin();;) {
            ...
            if (m_Rootmin + ((pChild->Hbnd() * iWidth) / NORMALIZATION) > ORIGIN_Y) {
                ...
                newRootmin = m_deGotoQueue.back().first;
                newRootmax = m_deGotoQueue.back().second;
...
void CDasherModel::ScheduleOneStep(dasherint y1, dasherint y2, int nSteps, int limX, bool bExact) {
    ...
    //  alpha = MAX_Y / (MAX_Y + (nSteps-1)*(y2-y1)) : (same eqn as old Dasher) more so! reversing ~~ 1/3 ideal speed,
    //  and maxes out at moderate dasherX. alpha = (y2-y1) / (MAX_Y*(nSteps-1) + y2-y1) : too slow forwards, reverses
    //  too quick
    // We are using:
    ...
    if (targetRange < 2 * static_cast<dasherint>(limX)) {
#ifdef DEBUG_DYNAMICS
        {
            ...
            std::cout << "Too fast at X " << (y2 - y1) / 2 << ": would enter " << bits << "b = " << (bits * nSteps)
                      << " in " << nSteps << "steps; will now enter ";
        }
#endif
        ...
        {
            const dasherint n = targetRange * (MAX_Y - 2 * static_cast<dasherint>(limX)),
...
#else
    m1 *= static_cast<Dasher::dasherint>(frac);
    m2 *= static_cast<Dasher::dasherint>(frac);
}
        ...
        { // begin block A (regardless of #ifdef)
            ...
            std::cout << " enters " << dActualBits << "b = " << (dActualBits * nSteps) << " in " << nSteps
                      << "steps, should be " << dDesiredBits << "=>" << (dDesiredBits * nSteps) << ", error "
                      << int(abs(dDesiredBits - dActualBits) * 100 / dDesiredBits) << "%" << std::endl;
            if (bExact) m1 = m1t, m2 = m2t; // overwrite approx values (we needed them somewhere!)
#endif
    ...
    void CDasherModel::ExpandNode(CDasherNode * pNode) {
        ...
        if (iExpect != pNode->GetChildren().size()) {
            std::cout << "(Note: expected " << iExpect << " children, actually created " << pNode->GetChildren().size()
                      << ")" << std::endl;
        }
#endif
    ...
    void CDasherModel::RenderToView(CDasherView * pView, CExpansionPolicy & policy) {
        ...
        CDasherNode* pOutput =
            pView->Render(m_Root, m_Rootmin + m_iDisplayOffset, m_Rootmax + m_iDisplayOffset, policy);
    ...
    void CDasherModel::ScheduleZoom(dasherint y1, dasherint y2, int nsteps) {
        ...
        for (int s = nsteps; nsteps > 1; s += (--nsteps)) {
            ...
            m_deGotoQueue.push_back(
                std::pair<myint, myint>(R1 + static_cast<myint>(dFrac * static_cast<double>(r1 - R1)),
                                        R2 + static_cast<myint>(dFrac * static_cast<double>(r2 - R2))));
        }
