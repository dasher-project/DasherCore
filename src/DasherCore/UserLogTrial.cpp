...
CUserLogTrial::CUserLogTrial(const std::string& strCurrentTrialFilename, CFileLogger* GlobalFileLogger)
    : m_strCurrentTrialFilename(strCurrentTrialFilename), m_pGlobalFileLogger(GlobalFileLogger) {}
...
CUserLogTrial::~CUserLogTrial() {
    ...
    for (unsigned int i = 0; i < m_vpNavCycles.size(); i++) {
        ...
        if (pCycle != NULL) {
            ...
            for (unsigned int j = 0; j < pCycle->vectorButtons.size(); j++) {
                CUserButton* pButton = (CUserButton*)pCycle->vectorButtons[j];
            ...
            for (unsigned int j = 0; j < pCycle->vectorMouseLocations.size(); j++) {
                CUserLocation* pLocation = (CUserLocation*)pCycle->vectorMouseLocations[j];
            ...
            for (unsigned int j = 0; j < pCycle->vectorNavLocations.size(); j++) {
                NavLocation* pLocation = (NavLocation*)pCycle->vectorNavLocations[j];
                if (pLocation != NULL) {
...
bool CUserLogTrial::HasWritingOccured() {
    ...
    if (m_vpNavCycles.size() > 0) return true;
    return false;
}
...
void CUserLogTrial::StartWriting() {
    ...
    if (m_pSpan != NULL) m_pSpan->Continue();

    // Start the task timer if we haven't already done so
    if (m_pSpan == NULL) m_pSpan = new CTimeSpan("Time", false);
    ...
    if (m_vpNavCycles.size() > 0) {
        ...
        if ((pLastLocation != NULL) && (pLastLocation->span != NULL)) pLastLocation->span->Continue();
    }
...
void CUserLogTrial::AddSymbols(Dasher::VECTOR_SYMBOL_PROB* vpNewSymbolProbs, eUserLogEventType iEvent) {
...
void CUserLogTrial::Done() {
    ...
    if ((m_pSpan != NULL) && (!m_pSpan->IsStopped())) m_pSpan->Stop();
}
...
void CUserLogTrial::AddMouseLocationNormalized(int iX, int iY, bool bStoreIntegerRep, float dNats) {
    ...
    if ((m_sCanvasCoordinates.bottom == 0) && (m_sCanvasCoordinates.left == 0) && (m_sCanvasCoordinates.right == 0) &&
...
void CUserLogTrial::StopPreviousTimer() {
    ...
    if (m_vpNavCycles.size() > 0) {
        NavLocation* pLastLocation = GetCurrentNavLocation();
        if ((pLastLocation != NULL) && (pLastLocation->span != NULL)) pLastLocation->span->Stop();
    }
}
...
std::string CUserLogTrial::GetStatsXML(const std::string& strPrefix, const std::string& strText, CTimeSpan* pSpan,
                                       double dAvgBits, int iButtonCount, double dTotalBits) {
    std::string strResult = "";
    ...
    if (m_pSpan != NULL) strResult += m_pSpan->GetXML(strPrefixTabTab);
...
int CUserLogTrial::GetButtonCount() {
    ...
    for (VECTOR_NAV_CYCLE_PTR::iterator it(m_vpNavCycles.begin()); it != m_vpNavCycles.end(); ++it)
        for (VECTOR_USER_BUTTON_PTR::iterator it2((*it)->vectorButtons.begin()); it2 != (*it)->vectorButtons.end();
             ++it2)
            iCount += (*it2)->GetCount();
...
NavCycle* CUserLogTrial::GetCurrentNavCycle() {
    if (m_vpNavCycles.size() <= 0) return NULL;
...
std::string CUserLogTrial::GetNavCyclesXML(const std::string& strPrefix) {
    ...
    for (unsigned int i = 0; i < m_vpNavCycles.size(); i++) {
        ...
        if (pCycle != NULL) {
            ...
            if (pCycle->pSpan != NULL) strResult += pCycle->pSpan->GetXML(strPrefixTabTabTab);
            ...
            if (pCycle->vectorNavLocations.size() > 0) {
                ...
                for (unsigned int j = 0; j < pCycle->vectorNavLocations.size(); j++) {
                    ...
                    if (pLocation != NULL) strResult += GetLocationXML(pLocation, strPrefixTabTabTabTab);
                }
                strResult += strPrefixTabTabTab;
                strResult += "</Locations>\n";
            }
    ...
    for (VECTOR_STRING_ITER iter = vNavs.begin(); iter < vNavs.end(); iter++) {
        ...
        if (strLocations.length() > 0) {
            ...
            for (VECTOR_STRING_ITER iter2 = vLocations.begin(); iter2 < vLocations.end(); iter2++) {
                ...
                pLocation->span = NULL;
                std::string strLocTime = XMLUtil::GetElementString("Time", *iter2);
                pLocation->span = new CTimeSpan("Time", strLocTime);
                ...
                for (VECTOR_STRING_ITER iter3 = vAdded.begin(); iter3 < vAdded.end(); iter3++) {
                    ...
                    if (pLocation->pVectorAdded != NULL) pLocation->pVectorAdded->push_back(sAdd);
    ...
    for (VECTOR_NAV_CYCLE_PTR_ITER iter = m_vpNavCycles.begin(); iter < m_vpNavCycles.end(); iter++) {
        ...
        if (*iter != NULL) {
            for (VECTOR_USER_LOCATION_PTR_ITER iter2 = (*iter)->vectorMouseLocations.begin();
                 iter2 < (*iter)->vectorMouseLocations.end(); iter2++) {
                if (*iter2 != NULL) strResult += (*iter2)->GetTabMouseXY(bReturnNormalized);
            }
        }
...
DENSITY_GRID CUserLogTrial::MergeGrids(int iGridSize, DENSITY_GRID ppGridA, DENSITY_GRID ppGridB) {
    ...
    if ((ppGridA == NULL) && (ppGridB == NULL)) return ppResult;
