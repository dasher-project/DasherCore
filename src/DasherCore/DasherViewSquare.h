...
class Dasher::CDasherViewSquare : public CDasherView {
  public:
    ...
    void DasherSpaceArc(myint cy, myint r, myint x1, myint y1, myint x2, myint y2, const ColorPalette::Color& color,
                        int iLineWidth) const override;
  ...
  private:
    ...
    void TruncateTri(myint x, myint y1, myint y2, myint midy1, myint midy2, const ColorPalette::Color& fillColor,
    ...
    class CTextString {
      public: // to CDasherViewSquare...
        ...
        CTextString(CDasherScreen::Label* pLabel, screenint x, screenint y, int iSize,
                    const ColorPalette::Color& iColor)
    ...
    ///
    CTextString* DasherDrawText(myint iDasherMaxX, myint iDasherMidY, CDasherScreen::Label* pLabel,
    ...
    void DisjointRender(CDasherNode* Render, myint y1, myint y2, CTextString* prevText, CExpansionPolicy& policy,
                        double dMaxCost, CDasherNode*& pOutput);

    void DasherDrawCube(myint iDasherMaxX, myint iDasherMinY, myint iDasherMinX, myint iDasherMaxY,
                        CubeDepthLevel nodeDepth, CubeDepthLevel parentDepth, const ColorPalette::Color& Color,
                        const ColorPalette::Color& outlineColor, int iThickness,
                        ScreenRegion* parentScreenBounds) const;
    ...
    void NewRender(CDasherNode* pCurrentNode, myint y1, myint y2, CTextString* pPrevText, CExpansionPolicy& policy,
                   double dMaxCost, CDasherNode*& pCurrentTopCenterNode, CubeDepthLevel nodeDepth,
                   CubeDepthLevel parentDepth, ScreenRegion parentScreenBounds);
    ...
    /// screen coordinate (linear in screen space, -ive x = offscreen) - i.e. pixel coordinate = scale({x,y}map(dasher
    /// coord)))
