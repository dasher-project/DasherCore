...
CAlphabetManager::CAlphabetManager(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface,
    ...
    : m_pBaseGroup(NULL), m_pInterface(pInterface), m_pLanguageModel(nullptr), m_pNCManager(pNCManager),
      m_pAlphabet(pAlphabet), m_pSettingsStore(pSettingsStore), m_pLastOutput(NULL) {
    m_pSettingsStore->OnPreParameterChange.Subscribe(
        this, [this](Parameter parameter, const std::variant<bool, long, std::string>& newValue) {
...
void CAlphabetManager::CreateLanguageModel() {
    ...
    CLanguageModel* lm =
        LMRegistry::instance().create(lmId, m_pSettingsStore, m_pAlphabet, &m_map, m_pAlphabet->iEnd - 1);
...
void CAlphabetManager::MakeLabels(CDasherScreen* pScreen) {
    ...
    for (vector<CDasherScreen::Label*>::iterator it = m_vLabels.begin(); it != m_vLabels.end(); it++)
        delete (*it);
    m_vLabels.clear();
    for (map<const SGroupInfo*, CDasherScreen::Label*>::iterator it = m_mGroupLabels.begin();
         it != m_mGroupLabels.end(); it++)
        delete it->second;
...
SGroupInfo* CAlphabetManager::copyGroups(const SGroupInfo* pBase, CDasherScreen* pScreen) {
    ...
    if (static_cast<int>(m_vLabels.size()) < pBase->iEnd) m_vLabels.resize(pBase->iEnd);
    ...
    if (pRes->strLabel.length()) m_mGroupLabels[pRes] = pScreen->MakeLabel(pRes->strLabel);
...
CWordGeneratorBase* CAlphabetManager::GetGameWords() {
    ...
    if (!m_pSettingsStore->GetStringParameter(SP_GAME_TEXT_FILE).empty()) {
        ...
        m_pInterface->FormatMessage(
            "Note: GameTextFile setting specifies game sentences file '%s' but this does not exist", gtf.c_str());
    }
...
void CAlphabetManager::WriteTrainFileFull(CDasherInterfaceBase* pInterface) {
    ...
    if (!strTrainfileContext.empty()) {
        ...
        if (delimiter.empty()) {
            ...
            char c = 33;
            while (strTrainfileContext.find(c) != strTrainfileContext.length())
                c++; // will terminate, context is ~~5 chars
            delimiter = string(&c, 1);
        }
        strTrainfileBuffer =
            m_pAlphabet->GetContextEscapeChar() + delimiter + strTrainfileContext + delimiter + strTrainfileBuffer;
        strTrainfileContext = "";
    }
...
void CAlphBase::Do() {
    if (m_pMgr->m_pLastOutput && m_pMgr->m_pLastOutput == Parent()) m_pMgr->m_pLastOutput = this;
...
pair<symbol, CLanguageModel::Context> CAlphabetManager::GetContextSymbols(CDasherNode* pParent, int iRootOffset,
                                                                          const CAlphabetMap* pAlphMap) {
    vector<symbol> vContextSymbols;
    bool bHaveFinalSymbol = true;
    ...
    return pair<symbol, CLanguageModel::Context>(bHaveFinalSymbol ? vContextSymbols[vContextSymbols.size() - 1] : 0,
                                                 iContext);
}
...
void CSymbolNode::GetContext(CDasherInterfaceBase* pInterface, const CAlphabetMap* pAlphabetMap,
                             vector<symbol>& vContextSymbols, int iOffset, int iLength) {
...
CDasherNode* CSymbolNode::RebuildSymbol(CAlphNode* pParent, symbol iNewSymbol) {
    if (iNewSymbol == this->iSymbol) {
        DASHER_ASSERT(offset() == pParent->offset() + numChars());
        return this;
    }
    return CAlphBase::RebuildSymbol(pParent, iNewSymbol);
}

const ColorPalette::Color& CSymbolNode::getLabelColor(const ColorPalette* colorPalette) {
    if (colorPalette == nullptr) return ColorPalette::noColor;
    const ColorPalette::Color& result =
        colorPalette->GetNodeLabelColor(m_pMgr->GetAlphabet()->getColorGroup(iSymbol),
                                        m_pMgr->GetAlphabet()->getColorGroupOffset(iSymbol), UseAltColor());
    return result != ColorPalette::undefinedColor ? result : ColorPalette::noColor;
}
...
const ColorPalette::Color& CSymbolNode::getOutlineColor(const ColorPalette* colorPalette) {
    if (colorPalette == nullptr) return ColorPalette::noColor;
    const ColorPalette::Color& result =
        colorPalette->GetNodeOutlineColor(m_pMgr->GetAlphabet()->getColorGroup(iSymbol),
                                          m_pMgr->GetAlphabet()->getColorGroupOffset(iSymbol), UseAltColor());
    return result != ColorPalette::undefinedColor ? result : ColorPalette::noColor;
}
...
const ColorPalette::Color& CSymbolNode::getNodeColor(const ColorPalette* colorPalette) {
    if (colorPalette == nullptr) return ColorPalette::noColor;
    return colorPalette->GetNodeColor(m_pMgr->GetAlphabet()->getColorGroup(iSymbol),
                                      m_pMgr->GetAlphabet()->getColorGroupOffset(iSymbol), UseAltColor());
}
...
void CAlphabetManager::IterateChildGroups(CAlphNode* pParent, const SGroupInfo* pParentGroup, CAlphBase* buildAround) {
    ...
    while (i < iMax) {
        ...
        // uint64_t is platform-dependently #defined in DasherTypes.h as an (unsigned) 64-bit int ("__int64" or "long
        // long int")
        unsigned int iLbnd =
            (((*pCProb)[iStart - 1] - (*pCProb)[iMin - 1]) * static_cast<uint64_t>(CDasherModel::NORMALIZATION)) /
            iRange;
        unsigned int iHbnd =
            (((*pCProb)[iEnd - 1] - (*pCProb)[iMin - 1]) * static_cast<uint64_t>(CDasherModel::NORMALIZATION)) / iRange;
        if (bSymbol) {
            ...
            pNewChild = (buildAround) ? buildAround->RebuildGroup(pParent, pCurrentNode)
                                      : CreateGroupNode(pParent, pCurrentNode);
...
void CSymbolNode::UntrainSymbol() {
    ...
    if (m_pMgr->m_pLastOutput == this) {
        ...
        if (tr.length() <= buf.length() && buf.substr(buf.length() - tr.length(), tr.length()) == tr) {
...
void CSymbolNode::SetFlag(int iFlag, bool bValue) {
    if ((iFlag & NF_COMMITTED) && bValue && !GetFlag(NF_COMMITTED | NF_GAME) &&
        ...
        if (Parent()) {
            ...
            // (Note: for first symbol after startup: parent is (root) group node, which'll have the alphabet default
            // context)
