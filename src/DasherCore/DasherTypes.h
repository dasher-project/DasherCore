...
namespace Options {
// Numbers should be applied to elements of the following two enumerations as these preferences may be stored to file.
// Constancy between versions is a good idea. It should *not* be assumed that the numbers map onto anything useful.
...
enum RenderingShapeTypes {
    DISJOINT_RECTANGLE = 0,
    OVERLAPPING_RECTANGLE = 1,
    TRIANGLE = 2,
    TRUNCATED_TRIANGLE = 3,
    QUADRIC = 4,
    CIRCLE = 5,
    CUBE = 6
};
...
namespace Keys {
// Virtual Keys that are used in the KeyDown/KeyUp Events. Suggestions/Intentions are marked and numbers are assigned
// based on old "int" based keys
enum VirtualKey {
...
struct SymbolProb {
  public:
    SymbolProb(symbol _sym, const std::string& sDisp, double _prob) : sym(_sym), strDisplay(sDisp), prob(_prob) {}
