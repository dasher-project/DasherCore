...
int Dasher::currentNumNodeObjects() {
    return iNumNodes;
}
...
CDasherNode::CDasherNode(int iOffset, CDasherScreen::Label* pLabel)
    : onlyChildRendered(NULL), m_iLbnd(0), m_iHbnd(CDasherModel::NORMALIZATION), m_pParent(NULL),
      m_iFlags(DEFAULT_FLAGS), m_iOffset(iOffset), m_pLabel(pLabel) {
    iNumNodes++;
}
...
// CDasherNode::GetContext(CDasherInterfaceBase *pInterface, const CAlphabetMap *pAlphabet, vector<symbol>
...
void CDasherNode::GetContext(CDasherInterfaceBase* pInterface, const CAlphabetMap* pAlphabetMap,
...
//       - the given iLbnd is equal to the the last childs iHbnd of the new parent(or equal to zero if the new parent
