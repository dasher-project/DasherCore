...
class COneButtonDynamicFilter : public CButtonMultiPress {
  public:
    ...
    virtual void KeyDown(unsigned long Time, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                         CDasherModel* pModel) override;
    virtual void KeyUp(unsigned long Time, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                       CDasherModel* pModel) override;

  private:
    unsigned int maxClickCount() override { return 2; } // double-click to reverse
    virtual void TimerImpl(unsigned long Time, CDasherView* pView, CDasherModel* m_pDasherModel,
