...
CFileLogger::CFileLogger(const std::string& strFilenamePath, eLogLevel iLogLevel, int iOptionsMask)
    ...
    if (iOptionsMask & logFunctionEntryExit) m_bFunctionLogging = true;
    if (iOptionsMask & logTimeStamp) m_bTimeStamp = true;
    if (iOptionsMask & logDateStamp) m_bDateStamp = true;
    if (iOptionsMask & logDeleteOldFile) m_bDeleteOldFile = true;
    if (iOptionsMask & logFunctionTiming) m_bFunctionTiming = true;
    if (iOptionsMask & logOutputScreen) m_bOutputScreen = true;
    ...
    if (m_bDeleteOldFile) {
        Dasher::FileUtils::WriteUserDataFile(strFilenamePath, "",
                                             false); // Delete file contents. Consistent with old behavior.
    }
}
...
CFileLogger::~CFileLogger() {
    ...
    for (const auto& [function_name, duration] : m_mapFunctionDuration) {
        Log("%-60s%20.2f%10.2f", eLogLevel::logNORMAL, function_name.c_str(), duration,
            static_cast<double>(duration) / max_duration * 100.0);
    }
}
...
void CFileLogger::SetFilename(const std::string& strFilename) {
    ...
    // to remember to delete the file before every new debugging run.
    if (m_bDeleteOldFile) {
        Dasher::FileUtils::WriteUserDataFile(strFilename, "",
                                             false); // Delete file contents. Consistent with old behavior.
    }
}
...
std::string CFileLogger::GetTimeDateStamp() {
    ...
    if (m_bTimeStamp || m_bDateStamp) {
        ...
        int milliseconds = static_cast<int>(
            std::chrono::time_point_cast<std::chrono::milliseconds>(timepoint).time_since_epoch().count() % 1000);
