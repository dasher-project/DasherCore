...
CTrainer::CTrainer(CMessageDisplay* pMsgs, CLanguageModel* pLanguageModel, const CAlphInfo* pInfo,
                   const CAlphabetMap* pAlphabet)
    ...
    if (syms.size() == 1)
        ...
        pMsgs->FormatMessage("Warning: faulty alphabet definition, escape sequence %s must be a single unicode "
                             "character. This may worsen Dasher's text prediction.",
...
class ProgressStream : public CAlphabetMap::SymbolStream {
  public:
    ProgressStream(std::istream& _in, CTrainer::ProgressIndicator* pProg, CMessageDisplay* pMsgs, off_t iStart = 0)
        : SymbolStream(_in, pMsgs), m_iLastPos(iStart), m_pProg(pProg) {}
