...
void CBasicLog::EndTrial() {
    ...
    oFile << "\"" << m_strStartDate << "\":\"" << GetDateStamp() << "\":" << m_iSymbolCount << ":" << m_dBits << ":"
          << m_iKeyCount << ":" << m_iInitialRate / 100.0 << ":"
          << m_pSettingsStore->GetLongParameter(LP_MAX_BITRATE) / 100.0 << ":\""
          << m_pSettingsStore->GetStringParameter(SP_INPUT_FILTER) << "\":\""
          << m_pSettingsStore->GetStringParameter(SP_ALPHABET_ID) << "\"" << std::endl;
...
std::string CBasicLog::GetDateStamp() {
    auto datestamp = std::chrono::system_clock::to_time_t(
        std::chrono::system_clock::now()); // This is a very long format, should probably be replaced using put_time
                                           // where its used.
