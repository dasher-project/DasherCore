...
static SModuleSettings sSettings[] = {
    ...
    /* TRANSLATORS: Double-clicks are special in some situations (they cause us to start reversing). This is the time in
       which the button must be pressed twice to count.*/
    ...
    /* TRANSLATORS: Backoff = reversing in Dasher to correct mistakes. This allows a single button to be dedicated to
       activating backoff, rather than using multiple presses of other buttons.*/
...
COneButtonDynamicFilter::COneButtonDynamicFilter(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface,
...
bool COneButtonDynamicFilter::DecorateView(CDasherView* pView, CDasherInput* pInput) {
    ...
    pScreen->DrawRectangle(x1, y1, x2, y2, ColorPalette::noColor, pView->GetNamedColor(NamedColor::selectionHighlight),
                           2);
    ...
    pScreen->DrawRectangle(x1, y1, x2, y2, ColorPalette::noColor, pView->GetNamedColor(NamedColor::selectionInactive),
                           1);
...
void COneButtonDynamicFilter::KeyDown(unsigned long Time, Keys::VirtualKey Key, CDasherView* pDasherView,
...
void COneButtonDynamicFilter::KeyUp(unsigned long Time, Keys::VirtualKey Key, CDasherView* pDasherView,
                                    CDasherInput* pInput, CDasherModel* pModel) {
    if (Key == Keys::Primary_Input && !m_pSettingsStore->GetBoolParameter(BP_BACKOFF_BUTTON))
        // mouse click - will be ignored by superclass method.
        // simulate press of button 2...
...
void COneButtonDynamicFilter::TimerImpl(unsigned long Time, CDasherView* m_pDasherView, CDasherModel* m_pDasherModel,
                                        CExpansionPolicy** pol) {
    OneStepTowards(m_pDasherModel, m_iTargetX[m_iTarget], m_iTargetY[m_iTarget], Time,
                   FrameSpeedMul(m_pDasherModel, Time));
}
