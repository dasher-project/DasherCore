...
void CDefaultFilter::GetUISettings(std::vector<Dasher::Parameter>& List) {
    ...
    AddSettings(List, {BP_DRAW_MOUSE_LINE, BP_DRAW_MOUSE, BP_CURVE_MOUSE_LINE, BP_START_MOUSE, BP_START_SPACE,
                       BP_TURBO_MODE, BP_AUTOCALIBRATE, BP_REMAP_XTREME, LP_START_MODE, BP_STOP_OUTSIDE, LP_LINE_WIDTH,
                       LP_TARGET_OFFSET, LP_GEOMETRY});
}
...
bool CDefaultFilter::DecorateView(CDasherView* pView, CDasherInput* pInput) {
    ...
    if (m_pSettingsStore->GetBoolParameter(BP_DRAW_MOUSE_LINE)) {
        ...
        double angle = (((Y[noOfPoints-1]>CenterXY[1])?1.5708:-1.5708) -
        atan((double)(Y[0]-CenterXY[1])/(double)X[0]))/(double)(noOfPoints-1); for(int i = 1; i < noOfPoints-1; ++i)
          {
    ...
    if (m_pStartHandler) bDidSomething = m_pStartHandler->DecorateView(pView) || bDidSomething;
...
void CDefaultFilter::ExecuteMovement(unsigned long Time, CDasherView* pView, CDasherModel* pModel, myint newX,
                                     myint newY) {
    ...
    if (!isPaused()) {
        ...
        if (dSpeedMul == 1.0) m_pAutoSpeedControl->SpeedControl(m_iLastX, m_iLastY, pView);
    }

    if (m_pStartHandler) m_pStartHandler->Timer(Time, m_iLastX, m_iLastY, pView);
}
...
void CDefaultFilter::KeyUp(unsigned long iTime, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                           CDasherModel* pModel) {
    if (Key == Keys::Secondary_Input || Key == Keys::Tertiary_Input || Key == Keys::Button_1) m_bTurbo = false;
}
        ...
        if (++m_iCounter > 20) {
            ...
            if (((m_iSum > 0) ? m_iSum : -m_iSum) > CDasherModel::MAX_Y / 2)
                m_pSettingsStore->SetLongParameter(
                    LP_TARGET_OFFSET, m_pSettingsStore->GetLongParameter(LP_TARGET_OFFSET) + ((m_iSum > 0) ? -1 : 1));
