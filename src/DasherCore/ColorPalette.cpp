...
ColorPalette::Color::Color(int Red, int Green, int Blue, int Alpha)
    : Red(Red), Green(Green), Blue(Blue), Alpha(Alpha) {}

ColorPalette::Color::Color(const std::string& HexString, const Color& defaultColor) {
    static const std::regex pattern(
        "#?([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})?"); // only instantiate once, thus static
...
ColorPalette::Color ColorPalette::Color::operator*(float x) const {
    return {static_cast<int>(static_cast<float>(Red) * x), static_cast<int>(static_cast<float>(Green) * x),
            static_cast<int>(static_cast<float>(Blue) * x), static_cast<int>(static_cast<float>(Alpha) * x)};
}
...
const ColorPalette::Color& ColorPalette::GetNamedColor(const NamedColor::knownColorName& NamedColor,
                                                       bool AskParent) const {
...
const ColorPalette::Color& ColorPalette::GetGroupColor(const std::string& GroupName, const bool& UseAltColor) const {
    ...
    if (const auto& search = GroupColors.find(GroupName); search != GroupColors.end()) {
        const Color& result =
            GetAltColor(search->second.groupColor.first, search->second.groupColor.second, UseAltColor);
...
const ColorPalette::Color& ColorPalette::GetGroupOutlineColor(const std::string& GroupName, const bool& UseAltColor,
                                                              bool UseDefaultColor) const {
    if (GroupName.empty()) return undefinedColor;

    if (const auto& search = GroupColors.find(GroupName); search != GroupColors.end()) {
        const Color& result =
            GetAltColor(search->second.groupOutlineColor.first, search->second.groupOutlineColor.second, UseAltColor);
        if (result != undefinedColor) return result;
    }
...
const ColorPalette::Color& ColorPalette::GetGroupLabelColor(const std::string& GroupName, const bool& UseAltColor,
                                                            bool UseDefaultColor) const {
    if (GroupName.empty()) return undefinedColor;

    if (const auto& search = GroupColors.find(GroupName); search != GroupColors.end()) {
        const Color& result =
            GetAltColor(search->second.groupLabelColor.first, search->second.groupLabelColor.second, UseAltColor);
        if (result != undefinedColor) return result;
    }
...
const ColorPalette::Color& ColorPalette::GetNodeColor(const std::string& GroupName, const int& nodeIndexInGroup,
    ...
    if (const auto& search = GroupColors.find(GroupName); search != GroupColors.end()) {
        const Color& color = GetAltColor(search->second.nodeColorSequence, search->second.altNodeColorSequence,
                                         UseAltColor, nodeIndexInGroup);
        if (color != undefinedColor) return color;
    }
...
const ColorPalette::Color& ColorPalette::GetNodeOutlineColor(const std::string& GroupName, const int& nodeIndexInGroup,
                                                             const bool& UseAltColor, bool UseDefaultColor) const {
    if (GroupName.empty()) return undefinedColor;

    if (const auto& search = GroupColors.find(GroupName); search != GroupColors.end()) {
        const Color& result = GetAltColor(search->second.nodeOutlineColorSequence,
                                          search->second.altNodeOutlineColorSequence, UseAltColor, nodeIndexInGroup);
        if (result != undefinedColor) return result;
    }
...
const ColorPalette::Color& ColorPalette::GetNodeLabelColor(const std::string& GroupName, const int& nodeIndexInGroup,
    ...
    if (const auto& search = GroupColors.find(GroupName); search != GroupColors.end()) {
        const Color& result = GetAltColor(search->second.nodeLabelColorSequence,
                                          search->second.altNodeLabelColorSequence, UseAltColor, nodeIndexInGroup);
        if (result != undefinedColor) return result;
    }
