...
CDasherViewSquare::CDasherViewSquare(CSettingsStore* pSettingsStore, CDasherScreen* DasherScreen,
...
CDasherNode* CDasherViewSquare::Render(CDasherNode* pRoot, myint iRootMin, myint iRootMax, CExpansionPolicy& policy) {
    ...
    if (m_pSettingsStore->GetLongParameter(LP_SHAPE_TYPE) == Options::DISJOINT_RECTANGLE) {
        ...
        if (iRootMin > visibleRegion.minY)
            DasherDrawRectangle(visibleRegion.maxX, visibleRegion.minY, visibleRegion.minX, iRootMin,
                                GetNamedColor(NamedColor::background), GetNamedColor(NamedColor::defaultOutline), 0);

        if (iRootMax < visibleRegion.maxY)
            DasherDrawRectangle(visibleRegion.maxX, iRootMax, visibleRegion.minX, visibleRegion.maxY,
                                GetNamedColor(NamedColor::background), GetNamedColor(NamedColor::defaultOutline), 0);

        // to left (greater Dasher X)
        if (iRootMax - iRootMin < visibleRegion.maxX)
            DasherDrawRectangle(visibleRegion.maxX, std::max(iRootMin, visibleRegion.minY), iRootMax - iRootMin,
                                std::min(iRootMax, visibleRegion.maxY), GetNamedColor(NamedColor::background),
                                GetNamedColor(NamedColor::defaultOutline), 0);

        // to right (margin)
        DasherDrawRectangle(0, visibleRegion.minY, visibleRegion.minX, visibleRegion.maxY,
                            GetNamedColor(NamedColor::background), GetNamedColor(NamedColor::defaultOutline), 0);

        // and render root.
        DisjointRender(pRoot, iRootMin, iRootMax, nullptr, policy, std::numeric_limits<double>::infinity(),
                       currentTopCenterNode);
        ...
        if (IsSpaceAroundNode(iRootMin, iRootMax))
            DasherDrawCube(visibleRegion.maxX, visibleRegion.minY, 0, visibleRegion.maxY, {-1, 0}, {-1, -1},
                           GetNamedColor(NamedColor::background), GetNamedColor(NamedColor::defaultOutline), 0,
                           nullptr);

        m_CrosshairCubeLevel = -1;
        NewRender(pRoot, iRootMin, iRootMax, nullptr, policy, std::numeric_limits<double>::infinity(),
                  currentTopCenterNode, {0, 0}, {-1, 0}, screenRegion);

        // to right (margin)
        DasherDrawCube(0, visibleRegion.minY, visibleRegion.minX, visibleRegion.maxY, {m_CrosshairCubeLevel, 0},
                       {-1, -1}, GetNamedColor(NamedColor::background), GetNamedColor(NamedColor::defaultOutline), 0,
                       nullptr);
        ...
        if (currentTopCenterNode) {
            ...
            DasherDrawRectangle(visibleRegion.maxX, visibleRegion.minY, 0, visibleRegion.maxY,
                                currentTopCenterNode->getNodeColor(m_pColorPalette),
                                GetNamedColor(NamedColor::defaultOutline), 0);
        ...
        } else // easy case, whole screen is white (outside root node, e.g. when starting)
        {
            Screen()->DrawRectangle(0, 0, Screen()->GetWidth(), Screen()->GetHeight(),
                                    GetNamedColor(NamedColor::background), GetNamedColor(NamedColor::defaultOutline),
                                    0);
        }
        NewRender(pRoot, iRootMin, iRootMax, nullptr, policy, std::numeric_limits<double>::infinity(),
                  currentTopCenterNode, {0, 0}, {0, 0}, screenRegion);
    }
...
CDasherViewSquare::CTextString* CDasherViewSquare::DasherDrawText(myint iDasherMaxX, myint iDasherMidY,
                                                                  CDasherScreen::Label* pLabel,
                                                                  const ColorPalette::Color& Color) const {
    screenint x, y;
    ...
    // const myint iSize = (std::min(iDasherMaxX * 3, (iMaxY * 3) / 2) + iMaxY) *
    // m_pSettingsStore->GetLongParameter(LP_DASHER_FONTSIZE) / iMaxY;
    ...
    const float iSize = std::min(fSize, fSize * (0.6f * static_cast<float>(iDasherMaxX) / CDasherModel::ORIGIN_X +
                                                 0.4f)); // linear function passing through (0, fSize/2.5) and
                                                         // (CDasherModel::ORIGIN_X, fSize), capped at fSize
...
void CDasherViewSquare::DoDelayedText(CTextString* pText, myint extrusionLevel, myint groupRecursionDepth) {
    ...
    screenint textInset =
        m_pSettingsStore->GetLongParameter(LP_OUTLINE_WIDTH) + m_pSettingsStore->GetLongParameter(LP_TEXT_PADDING);
    ...
    case Options::LeftToRight: {
        ...
        if (x < Screen()->GetWidth() && iRight >= 0) {
            if (extrudedText) {
                Screen()->Draw3DLabel(pText->m_pLabel, x, y - textDims.second / 2, textInset, Options::LeftToRight,
                                      extrusionLevel, groupRecursionDepth, pText->m_iSize, pText->m_Color);
            } else {
                Screen()->DrawString(pText->m_pLabel, x + textInset, y - textDims.second / 2, pText->m_iSize,
                                     pText->m_Color);
            }
        }
    ...
    case Options::RightToLeft: {
        ...
        if (iLeft < Screen()->GetWidth() && x >= 0) {
            if (extrudedText) {
                Screen()->Draw3DLabel(pText->m_pLabel, iLeft, y - textDims.second / 2, -textInset, Options::RightToLeft,
                                      extrusionLevel, groupRecursionDepth, pText->m_iSize, pText->m_Color);
            } else {
                Screen()->DrawString(pText->m_pLabel, iLeft - textInset, y - textDims.second / 2, pText->m_iSize,
                                     pText->m_Color);
            }
        }
        for (const auto& pChild : pText->m_children) {
    ...
    case Options::TopToBottom: {
        ...
        if (y < Screen()->GetHeight() && iBottom >= 0) {
            if (extrudedText) {
                Screen()->Draw3DLabel(pText->m_pLabel, x - textDims.first / 2, y, textInset, Options::TopToBottom,
                                      extrusionLevel, groupRecursionDepth, pText->m_iSize, pText->m_Color);
            } else {
                Screen()->DrawString(pText->m_pLabel, x - textDims.first / 2, y + textInset, pText->m_iSize,
                                     pText->m_Color);
            }
        }
        for (const auto& pChild : pText->m_children) {
    ...
    case Options::BottomToTop: {
        ...
        if (iTop < Screen()->GetHeight() && y >= 0) {
            if (extrudedText) {
                Screen()->Draw3DLabel(pText->m_pLabel, x - textDims.first / 2, iTop + textDims.second, textInset,
                                      Options::BottomToTop, extrusionLevel, groupRecursionDepth, pText->m_iSize,
                                      pText->m_Color);
            } else {
                Screen()->DrawString(pText->m_pLabel, x - textDims.first / 2, iTop - textInset, pText->m_iSize,
                                     pText->m_Color);
            }
        }
        for (const auto& pChild : pText->m_children) {
...
void CDasherViewSquare::TruncateTri(myint x, myint y1, myint y2, myint midy1, myint midy2,
                                    const ColorPalette::Color& fillColor, const ColorPalette::Color& outlineColor,
                                    int lineWidth) const {
...
void CDasherViewSquare::Circle(myint Range, myint y1, myint y2, const ColorPalette::Color& fillColor,
    ...
    if (y1 < visibleRegion.minY) {
        ...
        x1 = std::min(visibleRegion.maxX,
                      static_cast<myint>(sqrt(static_cast<double>(r * r - sq(cy - visibleRegion.minY)))));
    ...
    if (y2 > visibleRegion.maxY) {
        // intersect...
        x2 = std::min(visibleRegion.maxX,
                      static_cast<myint>(sqrt(static_cast<double>(r * r - sq(visibleRegion.maxY - cy)))));
        ...
        if (x2 == visibleRegion.maxX && x1 == visibleRegion.maxX) {
            ...
            DASHER_ASSERT(y1 == visibleRegion.minY);
            DasherDrawRectangle(visibleRegion.maxX, visibleRegion.minY, 0, visibleRegion.maxY, fillColor, outlineColor,
                                lineWidth);
            return;
        }
...
void CDasherViewSquare::DasherSpaceArc(myint cy, myint r, myint x1, myint y1, myint x2, myint y2,
                                       const ColorPalette::Color& color, int iLineWidth) const {
    point p;
    // start point
    Dasher2Screen(x1, y1, p.x, p.y);
    std::vector<point> pts;
    pts.push_back(p);
    // if circle goes behind crosshair and we want the point of max-x, force division into two sections with that point
    // as boundary
...
void CDasherViewSquare::Quadric(myint Range, myint lowY, myint highY, const ColorPalette::Color& fillColor,
                                const ColorPalette::Color& outlineColor, int lineWidth) const {
...
#define NUM_STEPS 40
    ...
    const DasherCoordScreenRegion visibleRegion = VisibleRegion();
    {
        myint x1(0), y1(highY), x2(static_cast<myint>(Range * RR2)),
            y2(static_cast<myint>(highY * RR2 + midY * (1.0 - RR2))), x3(Range), y3(midY);
        for (int i = 0; i <= NUM_STEPS; i++) {
            double f = i / static_cast<double>(NUM_STEPS), of = 1.0 - f;
            Dasher2Screen(
                std::min(visibleRegion.maxX, static_cast<myint>(of * of * x1 + 2.0 * of * f * x2 + f * f * x3)),
                std::max(
                    visibleRegion.minY,
                    std::min(visibleRegion.maxY, static_cast<myint>(of * of * y1 + 2.0 * of * f * y2 + f * f * y3))),
                p_array[i].x, p_array[i].y);
        }
    }
    {
        myint x1(Range), y1(midY), x2(static_cast<myint>(Range * RR2)),
            y2(static_cast<myint>(lowY * RR2 + midY * (1.0 - RR2))), x3(0), y3(lowY);
        for (int i = 0; i <= NUM_STEPS; i++) {
            double f = i / static_cast<double>(NUM_STEPS), of = 1.0 - f;
            Dasher2Screen(
                std::min(visibleRegion.maxX, static_cast<myint>(of * of * x1 + 2.0 * of * f * x2 + f * f * x3)),
                std::max(
                    visibleRegion.minY,
                    std::min(visibleRegion.maxY, static_cast<myint>(of * of * y1 + 2.0 * of * f * y2 + f * f * y3))),
                p_array[i + NUM_STEPS + 1].x, p_array[i + NUM_STEPS + 1].y);
        }
    }
...
void CDasherViewSquare::DisjointRender(CDasherNode* pRender, myint y1, myint y2, CTextString* pPrevText,
                                       CExpansionPolicy& policy, double dMaxCost, CDasherNode*& pOutput) {
    ...
    if (pRender->getLabel()) {
        ...
        CTextString* pText =
            DasherDrawText(y2 - y1, (ny1 + ny2) / 2, pRender->getLabel(), pRender->getLabelColor(m_pColorPalette));
    ...
    if (pOutput == pRender->Parent() && Range > CDasherModel::ORIGIN_X && y1 < CDasherModel::ORIGIN_Y &&
        y2 > CDasherModel::ORIGIN_Y) {
        pOutput = pRender;
    ...
    if (pRender->ChildCount() == 0) {
        ...
        DasherDrawRectangle(std::min(Range, visibleRegion.maxX), std::max(y1, visibleRegion.minY), 0,
                            std::min(y2, visibleRegion.maxY), pRender->getNodeColor(m_pColorPalette),
                            pRender->getOutlineColor(m_pColorPalette), 0);
        ...
        if (CDasherNode* pChild = pRender->onlyChildRendered) {
            ...
            if (newy1 < visibleRegion.minY && newy2 > visibleRegion.maxY) {
                ...
                if (newy2 - newy1 < visibleRegion.maxX) // fill in to it's left...
                    DasherDrawRectangle(std::min(Range, visibleRegion.maxX), std::max(y1, visibleRegion.minY),
                                        newy2 - newy1, std::min(y2, visibleRegion.maxY),
                                        pRender->getNodeColor(m_pColorPalette),
                                        pRender->getOutlineColor(m_pColorPalette), 0);
        ...
        if (!pRender->onlyChildRendered) {
            // render all children...
            myint lasty = y1;
            for (auto i = pRender->GetChildren().begin(); i != pRender->GetChildren().end(); ++i) {
                id++;
                ...
                if (newy1 < visibleRegion.minY && newy2 > visibleRegion.maxY &&
                    (!pRender->GetFlag(CDasherNode::NF_GAME) || pChild->GetFlag(CDasherNode::NF_GAME))) {
                    DASHER_ASSERT(dMaxCost == std::numeric_limits<double>::infinity());
                    ...
                    && newy1 <= visibleRegion.maxY && newy2 >= visibleRegion.minY)        // at least partly on screen
                {
                    ...
                    // fill in to its left
                    DasherDrawRectangle(std::min(y2 - y1, visibleRegion.maxX), std::max(newy1, visibleRegion.minY),
                                        std::min(newy2 - newy1, visibleRegion.maxX),
                                        std::min(newy2, visibleRegion.maxY), pRender->getNodeColor(m_pColorPalette),
                                        pRender->getOutlineColor(m_pColorPalette), 0);

                    if (std::max(lasty, visibleRegion.minY) <
                        newy1) // fill in interval above child up to the last drawn child
                        DasherDrawRectangle(std::min(Range, visibleRegion.maxX), std::max(lasty, visibleRegion.minY), 0,
                                            std::min(newy1, visibleRegion.maxY), pRender->getNodeColor(m_pColorPalette),
                                            pRender->getOutlineColor(m_pColorPalette), 0);
                    lasty = newy2;
                    DisjointRender(pChild, newy1, newy2, pPrevText, policy, dMaxCost, pOutput);
                } else {
                    ...
                    if (!pChild->GetFlag(CDasherNode::NF_SEEN)) pChild->DeleteChildren();
                }
            }
    ...
    if (m_pSettingsStore->GetLongParameter(LP_OUTLINE_WIDTH) &&
        !pRender->getOutlineColor(m_pColorPalette).isFullyTransparent()) {
        DasherDrawRectangle(std::min(Range, visibleRegion.maxX), std::max(y1, visibleRegion.minY), 0,
                            std::min(y2, visibleRegion.maxY), ColorPalette::noColor,
                            pRender->getOutlineColor(m_pColorPalette),
                            labs(m_pSettingsStore->GetLongParameter(LP_OUTLINE_WIDTH)));
    }
}

void CDasherViewSquare::DasherDrawCube(myint iDasherMaxX, myint iDasherMinY, myint iDasherMinX, myint iDasherMaxY,
                                       CubeDepthLevel nodeDepth, CubeDepthLevel parentDepth,
                                       const ColorPalette::Color& Color, const ColorPalette::Color& outlineColor,
                                       int iThickness, ScreenRegion* parentScreenBounds) const {
    ...
    Screen()->DrawCube(static_cast<float>(iScreenMinX) + (sizeX / 2.0f),
                       static_cast<float>(iScreenMinY) + (sizeY / 2.0f), sizeX, sizeY, nodeDepth, parentDepth, Color,
...
ColorPalette::Color CDasherViewSquare::SimulateTransparency(CDasherNode* pCurrentNode) const {
    ...
    ColorPalette::Color interpolatedColor =
        ColorPalette::Color::lerp(nodeColor, parentColor, static_cast<float>(nodeColor.Alpha) / 255.0f);
...
void CDasherViewSquare::NewRender(CDasherNode* pCurrentNode, myint y1, myint y2, CTextString* pPrevText,
    ...
    pCurrentNode->SetFlag(CDasherNode::NF_SUPER, (!IsSpaceAroundNode(y1, y2) &&
                                                  !pCurrentNode->getNodeColor(m_pColorPalette).isFullyTransparent()));
    ...
    if (pCurrentNode->getLabel()) {
        myint ny1 = std::min(visibleRegion.maxY, std::max(visibleRegion.minY, y1)),
              ny2 = std::min(visibleRegion.maxY, std::max(visibleRegion.minY, y2));
        CTextString* pText = DasherDrawText(y2 - y1, (ny1 + ny2) / 2, pCurrentNode->getLabel(),
                                            pCurrentNode->getLabelColor(m_pColorPalette));
    ...
    if (!pCurrentNode->getNodeColor(m_pColorPalette).isFullyTransparent()) {
        ...
        const ColorPalette::Color& fill_color = line_width < 0
                                                    ? ColorPalette::noColor
                                                    : (m_pSettingsStore->GetBoolParameter(BP_SIMULATE_TRANSPARENCY)
                                                           ? SimulateTransparency(pCurrentNode)
        ...
        case Options::OVERLAPPING_RECTANGLE:
            DasherDrawRectangle(std::min(Range, visibleRegion.maxX), std::max(y1, visibleRegion.minY), 0,
                                std::min(y2, visibleRegion.maxY), fill_color, outline_color, line_width);
            break;
        case Options::TRIANGLE:
        ...
        case Options::CIRCLE:
            ...
            break;
        case Options::CUBE:
            DasherDrawCube(std::min(Range, visibleRegion.maxX), std::max(y1, visibleRegion.minY), 0,
                           std::min(y2, visibleRegion.maxY), nodeDepth, parentDepth, fill_color, outline_color,
                           line_width, &parentScreenBounds);
            break;
        default:
            break;
        }
    ...
    if (CDasherNode* pChild = pCurrentNode->onlyChildRendered) {
        ...
        if ((newy1 < visibleRegion.minY && newy2 > visibleRegion.maxY)) {
            ...
            NewRender(pChild, newy1, newy2, pPrevText, policy, dMaxCost, pCurrentTopCenterNode, nextLevel, nodeDepth,
                      parentScreenBounds);
            return;
        }
    ...
    while (I != E) {
        ...
        if (newy1 <= visibleRegion.maxY && newy2 >= visibleRegion.minY) {
            ...
            if (newy2 - newy1 > m_pSettingsStore->GetLongParameter(LP_MIN_NODE_SIZE)) {
                ...
                NewRender(pChild, newy1, newy2, pPrevText, policy, dMaxCost, pCurrentTopCenterNode, nextLevel,
                          nodeDepth, parentScreenBounds);
            } else if (!pChild->GetFlag(CDasherNode::NF_SEEN))
            ...
            if (newy2 > visibleRegion.maxY && !pCurrentNode->GetFlag(CDasherNode::NF_GAME)) {
                ...
                if (newy1 < visibleRegion.minY)
                    pCurrentNode->onlyChildRendered = pChild; // previous children also offscreen!
...
void CDasherViewSquare::ComputeScaleFactor() {
    ...
    m_iXlogThres =
        CDasherModel::MAX_Y / 2; // threshold: DasherX's less than this are linear; those greater are logarithmic
    ...
    case Options::ScreenGeometry::squish_and_log:
        ...
        double dDasherXPerPixel(
            (dAspect < 1.0) ? (dMinXPerPixel + pow(dAspect, 3.0) * (dDesiredXPerPixel - dMinXPerPixel)) // tall+thin
                            : (1.0 / dScaleFactorY)); // square or wide+low
        iMarginWidth = static_cast<myint>(iMarginWidth /
...
void CDasherViewSquare::Dasher2Screen(myint iDasherX, myint iDasherY, screenint& iScreenX, screenint& iScreenY) const {
    ...
    case Options::LeftToRight:
        ...
        iScreenY = static_cast<screenint>(iScreenHeight / 2 +
    ...
    case Options::RightToLeft:
        ...
        break;
    case Options::TopToBottom:
        iScreenX = static_cast<screenint>(iScreenWidth / 2 +
                                          CustomIDivScaleFactor((iDasherY - CDasherModel::MAX_Y / 2) * iScaleFactorY));
...
void CDasherViewSquare::DasherLine2Screen(myint x1, myint y1, myint x2, myint y2, std::vector<point>& vPoints) const {
    if (x1 != x2 && y1 != y2) {
        ...
        if (m_pSettingsStore->GetLongParameter(LP_NONLINEAR_X) && (x1 > m_iXlogThres || x2 > m_iXlogThres)) {
            ...
            // broke out of loop. a straight line (x1,y1)-(x2,y2) on the screen is an accurate portrayal of a straight
            // line in Dasher-space.
