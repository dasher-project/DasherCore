...
static SModuleSettings sSettings[] = {
    ...
    /* TRANSLATORS: Multiple button presses are special (like a generalisation on double clicks) in some situations.
    This is the maximum time between two presses to count as _part_of_ a multi-press gesture (potentially more than two
    presses). */
...
CTwoButtonDynamicFilter::CTwoButtonDynamicFilter(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface,
...
void CTwoButtonDynamicFilter::ComputeLagBits() {
    m_dLagBits = m_pSettingsStore->GetLongParameter(LP_MAX_BITRATE) / 100.0 *
                 m_pSettingsStore->GetLongParameter(LP_DYNAMIC_BUTTON_LAG) / 1000.0;
}
...
void CTwoButtonDynamicFilter::KeyDown(unsigned long Time, Keys::VirtualKey Key, CDasherView* pView,
                                      CDasherInput* pInput, CDasherModel* pModel) {
...
void CTwoButtonDynamicFilter::KeyUp(unsigned long Time, Keys::VirtualKey Key, CDasherView* pView, CDasherInput* pInput,
                                    CDasherModel* pModel) {
    if (Key == Keys::Primary_Input && !m_pSettingsStore->GetBoolParameter(BP_BACKOFF_BUTTON)) {
        // mouse click - will be ignored by superclass method.
...
void CTwoButtonDynamicFilter::TimerImpl(unsigned long Time, CDasherView* m_pDasherView, CDasherModel* m_pDasherModel,
...
void CTwoButtonDynamicFilter::ActionButton(unsigned long iTime, Keys::VirtualKey Key, int iType, CDasherModel* pModel) {
    ...
    if (m_pSettingsStore->GetBoolParameter(BP_2B_INVERT_DOUBLE) && iType == 2 && Key >= Keys::Button_2 &&
        Key <= Keys::Button_4) { // double-press - go BACK in opposite direction,
