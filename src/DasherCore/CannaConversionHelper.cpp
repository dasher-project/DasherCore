...
CCannaConversionHelper::CCannaConversionHelper(CNodeCreationManager* pNCManager, const CAlphInfo* pAlphabet, int Type,
                                               int Order)
    ...
    for (int i = 0; i < dicnum; i++) {
        ...
        if (ret) std::cout << "Error loading:" << p << std::endl;
...
bool CCannaConversionHelper::Convert(const std::string& strSource, SCENode** pRoot) {
    ...
    if (strSource.size() == 0) return false;
    ...
    for (int i = nbun - 1; i >= 0; --i) {
        ...
        for (int j = 0; j < len; ++j) {
            ...
            *outbuf = '\0';
            ...
            if (strlen((char*)str_utf8)) vCandidates.push_back((char*)str_utf8);
...
void CCannaConversionHelper::ProcessCandidate(std::string strCandidate, SCENode* pRoot, SCENode* pTail) {
    ...
    while (iIdx < strCandidate.size()) {
        ...
        while (pCurrentChild) {
            if (strSymbol == pCurrentChild->pszConversion) break;
            pCurrentChild = pCurrentChild->GetNext();
        }

        if (!pCurrentChild) { // Need a new child
            pCurrentChild = new SCENode;
            if (pCurrentNode->GetChild()) pCurrentChild->SetNext(pCurrentNode->GetChild());
            if (pTail && (iIdx >= strCandidate.size())) pCurrentChild->SetChild(pTail);
...
void CCannaConversionHelper::AssignSizes(SCENode** pStart, Dasher::CLanguageModel::Context context, long normalization,
                                         int uniform, int iNChildren) {
