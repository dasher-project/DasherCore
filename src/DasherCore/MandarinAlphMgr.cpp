...
CMandarinAlphMgr::CMandarinAlphMgr(CSettingsStore* pSettingsStore, CDasherInterfaceBase* pInterface,
                                   CNodeCreationManager* pNCManager, const CAlphInfo* pAlphabet)
...
void CMandarinAlphMgr::InitMap() {
    ...
    for (const char i : additonalChars) {
        ...
        m_vGroupsByConversion[hashed].insert(
            static_cast<int>(m_vConversionsByGroup.size())); // identifies the new PY sound
        m_vConversionsByGroup.push_back(std::vector<symbol>(1, hashed));
        m_pPYgroups->iEnd++;
        m_pPYgroups->iNumChildNodes++;
    }
}
...
SGroupInfo* CMandarinAlphMgr::makePYgroup(const SGroupInfo* in) {
    ...
    for (int i = in->iStart; i < in->iEnd;) {
        if (!oldCh || i < oldCh->iStart) {
            ...
            // now, put in PY-group...
            // if (i!=m_pAlphabet->GetSpaceSymbol() && i!=m_pAlphabet->GetParagraphSymbol()) { // Pretty sure that these
            // characters do not exist in any group, so this should never happen in my understanding
            if (static_cast<int>(m_vConversionsByGroup.size()) == ret->iStart) {
    ...
    ret->iEnd =
        static_cast<int>(m_vConversionsByGroup.size()); // record all indices allocated to this group and descendants
...
CMandarinAlphMgr::CMandarinTrainer::CMandarinTrainer(CMessageDisplay* pMsgs, CMandarinAlphMgr* pMgr)
    ...
    if (trainStartSyms.size() == 1)
        ...
        m_pMsgs->FormatMessage("Warning: faulty alphabet definition: training-start delimiter %s must be a single "
                               "unicode character. May be unable to process training file.",
...
symbol CMandarinAlphMgr::CMandarinTrainer::getPYsym(bool bHavePy, const std::string& strPy, symbol symCh) {
    ...
    if (posPY.size() == 1) {
        ...
        if (bHavePy && m_pMgr->m_vGroupNames[pySym] != strPy)
            m_pMsgs->FormatMessage("Warning: training file contains character '%s' as member of group '%s', but no "
                                   "group of that name contains the character; ignoring group specifier",
                                   m_pInfo->GetDisplayText(symCh).c_str(), strPy.c_str());
        return pySym;
    }
    std::set<symbol> withName;
    if (bHavePy) {
        for (std::set<symbol>::iterator it = posPY.begin(); it != posPY.end(); it++)
            if (m_pMgr->m_vGroupNames[*it] == strPy) withName.insert(*it);
        if (withName.size() == 1)
            return *(withName.begin());
        else
            m_pMsgs->FormatMessage(
                (withName.empty())
                    ? "Warning: training file contains character '%s' as member of group '%s', but no group of that "
                      "name contains the character. Dasher will not be able to learn how you want to write this "
                      "character."
                    : "Warning: training file contains character '%s' as member of group '%s', but alphabet contains "
                      "several such groups. Dasher will not be able to learn how you want to write this character.",
                m_pInfo->GetDisplayText(symCh).c_str(), strPy.c_str());
    }
    return 0;
}
...
void CMandarinAlphMgr::CMandarinTrainer::Train(CAlphabetMap::SymbolStream& syms) {
    ...
    for (symbol sym; (sym = syms.next(m_pAlphabet)) != -1;) {
        if (sym == m_iStartSym) {
            if (sym != 0 || syms.peekBack() == m_pInfo->m_strConversionTrainStart) {
                if (bHavePy)
                    m_pMsgs->FormatMessage("Warning: in training file, annotation '<%s>' is followed by another "
                                           "annotation and will be ignored",
                                           strPy.c_str());
                strPy.clear();
                bHavePy = true;
                for (std::string s; (s = syms.peekAhead(m_pAlphabet)).length(); strPy += s) {
        ...
        if (sym) {
            if (symbol pySym = getPYsym(bHavePy, strPy, sym))
                ...
                unannotated.insert(sym); // no PY and unannotated -> warn user
    ...
    if (unannotated.size()) {
        ...
        const char* msg = ngettext(
            "In file %s, the following %i symbol appeared without annotations saying how it should be entered, but it "
            "can be entered in several ways. Dasher will not be able to learn how you want to enter this symbol:",
            "In file %s, the following %i symbols appeared without annotations saying how they should be entered, but "
            "each can be entered in several ways. Dasher will not be able to learn how you want to enter these "
            "symbols:",
            unannotated.size());
#else
        const char* msg = _("In file %s, the following %i symbols appeared without annotations saying how they should "
                            "be entered, but each can be entered in several ways. Dasher will not be able to learn how "
...
CDasherNode* CMandarinAlphMgr::CreateSymbolNode(CAlphNode* pParent, symbol iSymbol) {
    ...
    if (convs.size() > 1 || m_vLabels[iSymbol]) return CreateConvRoot(pParent, iSymbol);
    // elide CConvRoot...
...
CMandarinAlphMgr::CConvRoot::CConvRoot(int iOffset, CMandarinAlphMgr* pMgr, symbol pySym)
    ...
    DASHER_ASSERT(pMgr->m_vConversionsByGroup[pySym].size() > 1 || pMgr->m_vLabels[pySym]);
...
void CMandarinAlphMgr::CConvRoot::PopulateChildrenWithExisting(CMandSym* existing) {
    ...
    for (std::vector<std::pair<symbol, unsigned int>>::const_iterator it = m_vChInfo.begin(); it != m_vChInfo.end();
        ...
        iCum = iHbnd;
        CMandSym* pNewNode = (existing) ? existing->RebuildCHSymbol(this, it->first)
                                        : mgr()->CreateCHSymbol(this, this->iContext, it->first, m_pySym);
        pNewNode->Reparent(this, iLbnd, iHbnd);
    }
}

CMandarinAlphMgr::CMandSym* CMandarinAlphMgr::CreateCHSymbol(CDasherNode* pParent, CLanguageModel::Context iContext,
                                                             symbol iCHsym, symbol iPYparent) {
...
void CMandarinAlphMgr::CConvRoot::SetFlag(int iFlag, bool bValue) {
    if (iFlag == NF_COMMITTED && bValue && !GetFlag(NF_COMMITTED) && !GetFlag(NF_GAME) &&
        mgr()->m_pSettingsStore->GetBoolParameter(BP_LM_ADAPTIVE)) {
...
void CMandarinAlphMgr::GetConversions(std::vector<std::pair<symbol, unsigned int>>& vChildren, symbol pySym,
    ...
    if (long percent = m_pSettingsStore->GetLongParameter(LP_PY_PROB_SORT_THRES)) {
        ...
        const unsigned int uniform(
            static_cast<unsigned int>((m_pSettingsStore->GetLongParameter(LP_UNIFORM) * iNorm) / 1000));
        ...
        const unsigned int stop(
            static_cast<unsigned int>(iNorm - (iNorm * percent) / 100)); // intermediate values are uint64
    ...
    if (iRemaining) {
        unsigned int iEach(static_cast<unsigned int>(iRemaining) /
                           (static_cast<unsigned int>(convs.size()) - static_cast<unsigned int>(haveProbs.size())));
        for (std::vector<symbol>::const_iterator it = convs.begin(); it != convs.end(); it++) {
            if (haveProbs.count(*it) == 0) vChildren.push_back(std::pair<symbol, unsigned int>(*it, iEach));
        }
        ...
        for (std::vector<std::pair<symbol, unsigned int>>::iterator it = vChildren.end();
             iRemaining && it-- != vChildren.begin();) {
...
CDasherScreen::Label* CMandarinAlphMgr::GetCHLabel(int iCHsym) {
    ...
    if (iCHsym >= static_cast<int>(m_vCHLabels.size())) {
...
void CMandarinAlphMgr::CMandSym::RebuildForwardsFromAncestor(CAlphNode* pNewNode) {
    if (m_pyParent == 0) {
        ...
        if (possiblePinyin.size() > 1) {
            ...
            m_pyParent = *(possiblePinyin.begin());
    }
...
std::string CMandarinAlphMgr::CMandSym::trainText() {
    ...
    if (py.size() == 1) return s;
    // otherwise, ambiguous, record name
    if (!m_pyParent)
        return ""; // output nothing! TODO could reset context for what follows - but this really shouldn't ever happen?

    return mgr()->m_pAlphabet->m_strConversionTrainStart + mgr()->m_vGroupNames[m_pyParent] +
           mgr()->m_pAlphabet->m_strConversionTrainStop + s;
}
