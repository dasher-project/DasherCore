...
point CCircleStartHandler::CircleCenter(CDasherView* pView) {
    ...
    m_pView->Dasher2Screen(CDasherModel::ORIGIN_X, CDasherModel::ORIGIN_Y, m_screenCircleCenter.x,
                           m_screenCircleCenter.y);
    ...
    m_pView->Dasher2Screen(CDasherModel::ORIGIN_X,
                           CDasherModel::ORIGIN_Y +
                               (CDasherModel::MAX_Y * m_pSettingsStore->GetLongParameter(LP_CIRCLE_PERCENT)) / 100,
                           iEdgeX, iEdgeY);
...
bool CCircleStartHandler::DecorateView(CDasherView* pView) {
    ...
    if (m_pFilter->isPaused()) {
        pView->Screen()->DrawCircle(
            ctr.x, ctr.y, m_iScreenRadius,
            pView->GetNamedColor(bAboutToChange ? NamedColor::circleWaiting : NamedColor::circleStopped),
            pView->GetNamedColor(NamedColor::circleOutline), 1);
    } else {
        pView->Screen()->DrawCircle(ctr.x, ctr.y, m_iScreenRadius, ColorPalette::noColor,
                                    pView->GetNamedColor(NamedColor::circleOutline), bAboutToChange ? 3 : 1);
    }
...
void CCircleStartHandler::Timer(unsigned long iTime, dasherint mouseX, dasherint mouseY, CDasherView* pView) {
    ...
    const bool inCircleNow =
        dx * dx + dy * dy <= (m_iScreenRadius * m_iScreenRadius) && pView->Screen()->IsPointVisible(x, y);
...
void CCircleStartHandler::RegisterView(CDasherView* pView) {
    ...
    m_pView = pView;
    m_pView->OnGeometryChanged.Subscribe(this, [this]() { m_iScreenRadius = -1; });
